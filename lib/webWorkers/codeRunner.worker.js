...
self.addEventListener('message', ({ data: { type, payload } }) => {
  if (type === 'play') {
    ...
    runners.step(...runners.iterator, payload || false)
...
const runners = {
  ...
  newState (payload) {
    ...
    // for (const key in pick(this.currentState.globals, this.currentState)) {
    //   self[key] = this.currentState[key]
    // }
    self['frame'] = this.currentState['frame']
  },
  ...
  createRunner () {
    try {
      ...
      self.currentState = this.currentState
      this.iterator = [
        createIterator(
  ...
  runBot (it, i, returnValue) {
    ...
    const currentLocation = animals[active].current.location
    ...
    if (!running) {
      ...
      this.newState({
        animals: toggleActiveBot(
          this.currentState,
          !checkBounds(currentLocation, levelSize)
        )
      })
      return
    } else if (drawing) {
      ...
      if (shouldContinue) {
        ...
        return this.setFrameValue(
          { payload: [0] },
          {
            ...
            inDrawLoop: false,
            frame: numFrames
              ? this.currentState.frame % numFrames + 1
              : this.currentState.frame + 1
          },
          it,
          i
        )
  ...
  handleStepAction (action) {
    const { initialPainted, frame, numFrames } = this.currentState
      ...
      return [
        {
          ...
          inDrawLoop: false,
          frame: numFrames ? frame % numFrames + 1 : frame + 1
        }
      ]
    }
  ...
  setFrameValue (value, frame, it, i, newReturnValue, shouldContinue = true) {
    const { levelSize, active, steps, drawing, errorLog } = this.currentState
    ...
    if (drawing) {
      if (checkBounds(currentLocation, levelSize)) {
        this.newState({
          ...
          steps: steps + 1,
          activeLine: lineNum,
          animals: toggleActiveBot(frame, false),
          infoMessage: null
        })
      } else {
        const msg = `Out of Bounds at (${currentLocation[1]}, ${levelSize[0] -
          1 -
          currentLocation[0]})`
        this.newState({
          ...frame,
          ...
          steps: steps + 1,
          activeLine: lineNum,
          animals: toggleActiveBot(frame, true),
          infoMessage: msg,
          errorLog: errorLog && errorLog.length ? errorLog.concat(msg) : [msg]
        })
      }
    } else {
      if (!checkBounds(currentLocation, levelSize)) {
