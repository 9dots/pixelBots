                  ...
                  <Button
                    ...
                    onClick={
                      passwordSetting.value
                        ? actions.openPasswordModal(val)
                        : actions.getSignInToken(val.key)
                    }>
                    <Block ellipsis fs='s'>
                      {val.displayName}
                    </Block>
    ...
    controller: {
      * openPasswordModal ({ context, actions }, val) {
        yield context.openModal(() => (
          <PicturePasswordModal onClick={actions.getSignInToken} value={val} />
        ))
      },
      ...
      * getSignInToken ({ context, actions }, uid) {
        ...
        const { value } = yield context.fetch(
          `${process.env.CLOUD_FUNCTIONS}/weoAuth`,
          {
            method: 'POST',
