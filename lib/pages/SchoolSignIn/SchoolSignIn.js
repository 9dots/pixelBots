      ...
      <Block relative h='100%'>
        <Block {...disabledProps}>
          ...
          <Menu column overflowY='auto' h='296px'>
            {mapValues((val, key) => (
              <MenuItem  onClick={actions.getSignInToken(val.key)} key={val.key} align='start center' capitalize py='12' px='l'>
  ...
  render ({props, context}) {
    ...
    return (
    	<Block>
    		...
    		<Menu column h='328px'>
    			{mapValues(({classInfo, key}) => (
    				<MenuItem onClick={context.setUrl(`/schools/${props.schoolId}/${key}`)} key={key} align='start center' capitalize py='12' px='l'>
