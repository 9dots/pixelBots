...
import {MenuItem, Button, Grid} from 'vdux-containers'
import {Block, Menu, Icon} from 'vdux-ui'
import Loading from 'components/loading'
import PicturePasswordModal from 'components/PicturePasswordModal'
    ...
    return (
      <Block>
        <Block fontFamily='"Press Start 2P"' fs='m' textAlign='center' pb='l'>
      	...
      	<Block column minWidth={300} maxHeight={600} overflow='hidden' bgColor='white' boxShadow='0 1px 2px rgba(0,0,0,.3)' color='primary'>
      		{router(route, props)}
      	</Block>
      </Block>
    )
...
const StudentList = fire(({classId}) => ({
  ...
  passwordSetting: `/classes/${classId}/hasPicturePassword`
}))(component({
  render ({props, context, actions, state}) {
  	const {classInfo, schoolId, passwordSetting} = props
    const {loading} = state
      ...
      <Block relative h='100%'>
        <Block {...disabledProps}>
          ...
          <Grid itemsPerRow={cols} pb px align='center start'>
            {
              mapValues((val, key) => (
                <Button px='s' py m='s' textAlign='center' bold w={175} bgColor={colors[key % cols]} color='white' onClick={passwordSetting.value ? actions.openPasswordModal(val) : actions.getSignInToken(val.key)}> 
                  <Block ellipsis fs='s'>{val.displayName}</Block>
                </Button>
              ), sorted)
            }
          </Grid>
  ...
  controller: {
    ...
    * openPasswordModal({context, actions}, val) {
      yield context.openModal(() => <PicturePasswordModal redirect={actions} value={val} />)
    },
...
const ClassList = fire(({schoolId}) => ({
  ...
  const cols = getCols(schools.length)
    ...
    return (
    	<Block>
    		<Block fs='m' p='l' pb textAlign='center' bold>Find Your Class</Block>
        <Grid itemsPerRow={cols} pb px align='center start'>
          {
            mapValues(({classInfo, key}, i) => (
              <Button 
                ...
                py 
                bold
                // bgColor={colors[i % cols]} 
                // color='white' 
                >
                <Block ellipsis fs='s'>{classInfo.displayName}</Block>
              </Button>
            ), school.value)
          }
        </Grid>
