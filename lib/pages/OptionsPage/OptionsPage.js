...
export default component({
  render ({props, actions, children}) {
  	const {animals, inputType, type, updateGame, palette, capabilities = {}, lineLimit, stepLimit, stretchType} = props
	  ...
	  const animalType = animals[0].type
		...
		// const stretchGoal = lineLimit !== undefined 
		// 	? 'lineLimit'
		// 	: stepLimit !== undefined ?
	  ...
	  return (
	    <Block wide mx='auto' maxWidth={750}>
	      <Block>
	        ...
	        <DropdownField disabled={type !== 'write'} label='Stretch Goal' {...rowProps} borderBottomWidth='1'>
	          <StretchDropdown
	          	log={console.log(lineLimit, stepLimit)}
	            btn={<DropdownButton text={
	            	stretchType
	            		? stretchType === 'lineLimit' ? 'Lines' : 'Steps' 
	            		: 'None'
	            	} />
	            }
	            onSelect={actions.update((stretchType) => ({stretchType}))
	          	}/>
	            <Input hide={!stretchType} {...inputProps} onKeyUp={decodeValue(actions.setStretch)} value={props[stretchType]} />
	        </DropdownField>
