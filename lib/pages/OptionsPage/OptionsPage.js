...
export default component({
  render ({props, actions, children}) {
  	const {animals, inputType, type, updateGame, palette, capabilities = {}, stretch = {}} = props
	  ...
	  return (
	    <Block wide mx='auto' maxWidth={750}>
	      <Block>
	        ...
	        <DropdownField disabled={type !== 'write'} label='Stretch Goal' {...rowProps} borderBottomWidth='1'>
	          <StretchDropdown
	            btn={<DropdownButton text={
	            	stretch.type
	            		? stretch.type === 'lineLimit' ? 'Lines' : 'Steps'
	            		: 'None'
	            	} />
	            }
	            onSelect={actions.update((type) => ({stretch: {type, value: null}}))
	          	}/>
	      ...
	      <DropdownField label='Sound' {...rowProps} borderBottomWidth={1}>
	      	<Table w='60%' ml={-12} borderCollapse='collapse' border='1px solid divider'>
	      		{
              sounds.map((sound, i) => {
                return (
                  <TableRow bgColor={i % 2 ?  'white' : '#FAFAFA'}>
                      ...
                      <Checkbox {...checkProps}
                        label={sound}
                        checked={props.capabilities[sound]}
                        onChange={updateGame({
                          capabilities: {
                            ...capabilities,
