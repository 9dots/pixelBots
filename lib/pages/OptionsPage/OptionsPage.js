          ...
          <DropdownField label='Code Type' {...rowProps}>
            <CodeDropdown
              ...
              setInputType={actions.update(type => ({
                ...
                capabilities: {
                  ...capabilities,
                  ifStatement: type === 'code' && capabilities.ifStatement ? [true] : null
                },
          ...
          <DropdownField label='Structural' {...rowProps}>
            <Table
              ...
              {structural.map(({key, label}, i) => {
                const disabled = inputType === 'icons' && key === 'ifStatement'
                return <TableRow opacity={disabled ? 0.4 : 1} key={key} bgColor={!(i % 2) ? 'white' : '#FAFAFA'}>
                  <TableCell px py='s'>
                    <Checkbox
                      disabled={disabled}
                      {...checkProps}
