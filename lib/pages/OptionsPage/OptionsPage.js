...
export default component({
  render ({props, actions, children}) {
    const {
      animals, inputType, showModal, type, updateGame, palette,
      capabilities = {}, stretch = {}, inventorySize = 'None',
      wrappedMovement
    } = props

    if (!animals) return <span />

    const hard = stretch.hard
      ...
      <Block wide mx='auto' maxWidth={750}>
        <Block>
          ...
          <DropdownField label='Paints' {...rowProps}>
            <Table w='60%' ml={-12} borderCollapse='collapse' border='1px solid divider'>
            {
              ...
              <TableRow bgColor='#FAFAFA'>
                <TableCell py='s' px fontFamily='monospace'>
                  <Checkbox {...checkProps}
                    label={<Block my={-4} align='start center'>Toggle <BlockIcon ml='s' type='toggle' color='#555' /> </Block>}
          ...
          <DropdownField label='Memory' {...rowProps} >
            <Table w='60%' ml={-12} border='1px solid divider'>
              <TableRow>
                <TableCell py='s' px fontFamily='monospace' align='start center'>
                  <Block mr>Memory Size:</Block>
                </TableCell>
                <TableCell>
                  <Dropdown zIndex={999} btn={<DropdownButton text={inventorySize}/>}>
                    <MenuItem onClick={updateGame({inventorySize: null})}>None</MenuItem>
                    {range(1, 11).map(val => (
                      <MenuItem key={val} onClick={updateGame({inventorySize: val})}>{val}</MenuItem>
                    ))}
                  </Dropdown>
                </TableCell>
              </TableRow>
              <TableRow>
                <TableCell py='s' px fontFamily='monospace'>
                  <Checkbox {...checkProps} pb checked={!!wrappedMovement} onChange={updateGame({
                    wrappedMovement: wrappedMovement ? null : true
                  })} label={<Text>wrapped movement</Text>} />
                </TableCell>
              </TableRow>
              <TableRow>
                <TableCell py='s' px fontFamily='monospace'>
                  <Checkbox {...checkProps} pb checked={!!props.numberGrid} onChange={updateGame({
                    numberGrid: props.numberGrid ? null : true
                  })} label={<Text>number grid</Text>} />
                </TableCell>
              </TableRow>
              <TableRow>
                <TableCell py='s' px fontFamily='monospace'>
                  <Checkbox {...checkProps} pb checked={!!capabilities.pickUp} onChange={updateGame({
                    capabilities: {
                      ...capabilities,
              ...
              <TableRow>
                <TableCell py='s' px fontFamily='monospace'>
                  <Checkbox {...checkProps} pb checked={!!capabilities.place} onChange={updateGame({
                    capabilities: {
                      ...capabilities,
              ...
              {(
                ['pickUpMoveUp', 'pickUpMoveRight', 'pickUpMoveDown', 'pickUpMoveLeft'].map((cap) => (
                  <TableRow>
                    <TableCell py='s' px fontFamily='monospace'>
                      <Checkbox {...checkProps} pb checked={!!capabilities[cap]} onChange={updateGame({
                        capabilities: {
                          ...capabilities,
                          [cap]: capabilities[cap] ? null : true
                        }
                      })} label={<Text>{cap}</Text>} />
                    </TableCell>
                    <TableCell py='s' pl>
                      <Toggle
                        float='right'
                        checked={Array.isArray(capabilities[cap])}
                        disabled={!capabilities[cap]}
                        opacity={capabilities[cap] ? 1 : .4}
                        pointerEvents={capabilities[cap] ? 'all' : 'none'}
                        onChange={updateGame({
                          capabilities: {
                            ...capabilities,
                            [cap]: Array.isArray(capabilities[cap]) ? true : [true]
                          }
                        })} />
                    </TableCell>
                  </TableRow>
  ...
  controller: {
    ...
    * togglePalette ({actions, props, context}) {
      ...
      const isPainted = Object.keys(initialPainted).length
        || Object.keys(targetPainted).length
      ...
      if (isPainted) {
        yield context.openModal(() =>
          <ConfirmModal
            ...
            body={<Block lh='1.5em' mx={-16}>
              Changing the palette will clear <Text bold italic color='red'> everything </Text> you've painted on your <Text bold italic color='red'>start and target </Text> grids.  After you do this, make sure any start code you've added still makes sense with the new palette.</Block>
            }
            onSubmit={toggle}
            />
        )
