	  ...
	  return (
	    <Block wide mx='auto' maxWidth={750}>
	      <Block bgColor='white'>
	        ...
	        <DropdownField label='Paint Colors' {...rowProps}>
	        	<Toggle
	        		checked={capabilities.paint[0].length > 2}
	        		onChange={updateGame({
	        			capabilities: {
	        				...capabilities,
	        				paint: capabilities.paint[0].length === 2
	        					? [palette]
	        					: [blackAndWhite]
	        			}
	        		})} />
	        </DropdownField>
	        <DropdownField label='Movement' {...rowProps}>
	        	<Table>
	        		<TableHeader>
	        			<TableCell>
	        				Direction
		        	...
		        	{
		        		['up', 'down', 'left', 'right', 'forward', 'turnLeft', 'turnRight'].map(dir => (
		        			<TableRow>
		        				<TableCell>
		        					<Checkbox checked={!!capabilities[dir]} onChange={updateGame({
		        						capabilities: {
		        							...capabilities,
		        				...
		        				<TableCell>
		        					<Checkbox checked={Array.isArray(capabilities[dir])} disabled={!capabilities[dir]} onChange={updateGame({
		        						capabilities: {
		        							...capabilities,
	        ...
	        <DropdownField label='Structural' {...rowProps}>
	        	<Table>
	        		...
	        		<TableRow>
	        			<TableCell>
	        				<Checkbox checked={!!capabilities.if_color} onChange={updateGame({
	        					capabilities: {
	        						...capabilities,
	        						if_color: capabilities.if_color ? false : [true]
	        					}
	        				})} label='Conditionals' />
	        			</TableCell>
	        		</TableRow>
	        		<TableRow>
	        			<TableCell>
	        				<Checkbox checked={!!capabilities.repeat} onChange={updateGame({
	        					capabilities: {
	        						...capabilities,
