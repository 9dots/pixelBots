...
export default component({
	* onCreate ({props, context, actions}) {
	  if (!props.searchType) {
	    yield context.setUrl('/search/playlists')
	  }
	  if (props.searchQ) {
  ...
  render ({props, state, actions, context}) {
	  ...
	  const {searchType = 'playlists', searchQ = ''} = props
	  const {uid} = context

	  return (
	    <Block>
		    <PixelGradient h={250} mb={0}>
		    	<Block
		    ...
		    <Block pb='xl'>
		      <SearchResults key={searchKey} tab={searchType} searchKey={searchKey} searchQ={searchQ} />
		    </Block>
		  </Block>
	  )
  },
