    ...
    return items && Object.keys(items).length ? (
      ...
      <Block column align='start center'>
        <EmptyState
          icon='view_list'
          title='Challenges'
          description='All the playlists you create will appear here. Click the Create button in the sidebar to get started making your first playlist!' />
        <CreateButton mine={mine} />
      </Block>
    )
  },
  stats: function (params, props) {
    return <UserStats {...props} {...params} />
  },
...
const Profile = fire(props => ({
  thisProfile: !props.mine && `/users/${props.thisProfileId}`
}))(
  component({
    initialState: {
      selected: []
    ...
    render ({ props, context, actions, state }) {
      ...
      if (loading || (!value && !mine)) return <Loading />
      ...
      return (
        <Block flex column wide tall>
          <Block
            ...
            bgColor='#F0F0F0'
            overflowY='auto'>
            <ProfileNav isMine={mine} thisProfile={profile} bgColor='#FAFAFA' />
    ...
    * onUpdate (prev, { props, context }) {
      if (prev.props.thisProfile.loading && !props.thisProfile.loading) {
        if (!props.mine && !props.thisProfile.value) {
          yield context.setUrl('/404', true)
        }
      }
    },
    ...
    reducer: {
      toggleSelected: (state, key) => ({
        selected: maybeAddToArray(key, state.selected)
      }),
  ...
  render ({ props, context }) {
    ...
    return (
      <Button
        ...
        borderColor='blue'
        fs='xs'
        onClick={context.openModal(() => <CreateModal />)}
        {...rest}
