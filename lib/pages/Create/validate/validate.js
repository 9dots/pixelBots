...
import {getIterator, createFrames} from 'utils/frameReducer'
 ...
 */
 ...
 function testCode (sequence, capabilities, state) {
   return () => {
     try {
       const it = getIterator(sequence, animalApis(capabilities, 0))()
       return {valid: true}
     } catch (e) {
 ...
 export default function (state, next) {
   const reqs = {
     ...
     preview: [
       level.create,
       state.type === 'write' ? level.painted : valid,
       state.type === 'read' ? () => level.read({startCode: sequenceToCode(state.animals[0].sequence)}) : valid
     ],
     solution: [
       level.create,
     ...
     publish: [
       level.create,
       state.advanced ? level.initialPainted : level.painted,
