...
export default function (state, next) {
    ...
    publish: [
      ...
      state.type === 'read' || state.type === 'debug'
        ? () =>
          level.read({ startCode: sequenceToCode(state.animals[0].sequence) })
        : valid,
      state.type === 'write' && state.advanced ? level.initialPainted : valid,
      state.type === 'write' && !state.advanced ? level.painted : valid,
      state.advanced
        ? () =>
          level.solution({
            solution: sequenceToCode(state.solution[0].sequence)
          })
        : valid,
      state.advanced
        ? testCode(
          state.solution && state.solution[0].sequence,
          state.capabilities,
          state
        )
        : valid
  ...
  if (results.some(r => r.valid === false)) {
    const { message } = flatten(
      results.filter(r => r.valid === false).map(r => r.errors)
    )[0]
