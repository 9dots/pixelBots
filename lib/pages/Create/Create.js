...
const router = enroute({
  options: (params, props) => (
    <CreateLayout {...props} {...props.newGame.value} step='options'>
      <OptionsPage {...props.newGame.value} {...omit('title', props)}>
        {props.btn}
      </OptionsPage>
    </CreateLayout>
  ),
  ...
  preview: (params, props) => (
    <CreateLayout
      {...props}
      {...props.newGame.value}
      step='preview'
      validate=''>
      <GameLoader initialData={props.newGame.value} {...props} />
    </CreateLayout>
  ),
  solution: (params, props) => (
    <CreateLayout {...props} {...props.newGame.value} step='solution'>
      <GameLoader initialData={props.newGame.value} {...props} />
    </CreateLayout>
  ),
  publish: (params, props) => (
    <CreateLayout {...props} {...props.newGame.value} step='publish'>
      <PublishPage
        data={props.newGame.value}
...
export default fire(props => ({
  ...
  component({
    ...
    * onUpdate (prev, { props, context, state, actions }) {
    ...
    controller: {
      * updateGame ({ actions, context, props }, data) {
        const ref = props.new
          ? `/drafts/${props.draftID}`
          : `/games/${props.draftID}`
