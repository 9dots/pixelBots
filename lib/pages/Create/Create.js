...
export default fire((props) => ({
  ...
  controller: {
  	...
  	* publish ({context, props, state}) {
  		...
  		yield context.firebaseUpdate(`/games/${props.draftID}`, {
  			...props.newGame.value,
  			lastEdited: Date.now()
  		})
  		yield context.firebaseUpdate(`/games/${props.draftID}/meta`, {
  			...meta,
  			animals: props.newGame.value.animals.map(animal => animal.type)
  		})
  		yield context.setUrl(`/${context.username}/authored/challenges`)
  	},
