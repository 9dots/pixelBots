...
function getItems (profileGames = {}, profilePlaylists = {}) {
  // const allGames = mapValues((game) => ({...game, type: 'game'}), profileGames) || []
  // const games = allGames.filter((game) => !game.isInPlaylist)
  return mapValues((playlist) => ({...playlist, type: 'playlist'}), profilePlaylists)
...
export default component({
	* onCreate ({props, context}) {
	  ...
	  if (!mine) {
	    return yield context.setUrl(`/${username}`, true)
	  }
	  if (!subcategory) {
	    return yield context.setUrl(`/${username}/${category}/inProgress`, true)
	  }
	},
  render ({props, actions}) {
    if (!props.subcategory) return <span/>
	  return (
