...
export default fire((props) => ({
	    ...
	    <Layout
	      ...
	      <Menu mx='20px' column>
	        <Block
	          ...
	          <Text minWidth={colWidth} w={colWidth}>ANIMAL</Text>
  ...
  controller: {
	  ...
	  * play ({props, context}, current = 0) {
			const {url, userProfile, activeKey, imageUrl} = props
			const {uid} = context
			const {update} = parseQs(url.split('?')[1])

    	yield context.firebaseUpdate(`/users/${uid}/lists/${activeKey}`, {
    		playlistKey: activeKey,
    		lastEdited: Date.now(),
    		updateUrl: update || null,
    		current
    	})
	    ...
	    yield context.setUrl(`/playSequence/${activeKey}`)
	  },
