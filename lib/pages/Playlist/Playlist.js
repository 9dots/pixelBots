  ...
  controller: {
    * onGameComplete ({props, context, actions}) {
      const {sequence, current, savePath, playlist} = props
			const {completedChallenges = [], update} = playlist.value
      const {uid} = context
      ...
      if (equal(completed, sequence)) {
		...
		* awardBadges ({props, context, actions}, completed) {
			const {sequence, current, playlist, game, saved} = props
			const {completedChallenges = []} = playlist.value
			const {uid} = context
			const {type = 'write', stretch = {}} = game.value
			const {steps = 0, badges = {}, meta = {}} = saved.value
			const loc = linesOfCode(saved.value.animals[0].seqeuence)
			...
			if (type === 'write') {
				yield maybeAwardBadge('lineLimit', loc <= stretch.value)
				yield maybeAwardBadge('stepLimit', steps <= stretch.value)
			} else if (type === 'read') {
				const {invalidCount = 0} = saved.value
				yield maybeAwardBadge('errorLimit', invalidCount <= 3)
			} else if (type === 'debug') {
				const {stretch = {}} = game.value
				const {modifications = 0} = meta
				yield maybeAwardBadge('modLimit', modifications <= stretch.value)
			}

			function maybeAwardBadge (name, comp) {
				if (stretch.type === name && comp) {
					return actions.awardBadge(name, !badges[name])
				}
			}
		},
