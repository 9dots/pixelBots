...
const Playlist = fire((props) => ({
  ...
  render ({props, state, actions}) {
	  ...
	  return (
	    ...
	    <View {...props} savePath={state.savePath} {...playlistValue} current={props.current} next={actions.next} prev={actions.prev} />
	  )
  },
  ...
  controller: {
    * onGameComplete ({props, context}) {
      ...
      const {uid} = context
			...
			if (completedChallenges.indexOf(sequence[current]) === -1) {
				yield context.firebaseTransaction(
      ...
      yield context.firebaseUpdate(savePath + '/completedChallenges', union(completedChallenges, [sequence[current]]))

      yield context.setUrl(`${context.url}/results`)

      if (sequence.filter(s => completedChallenges.indexOf(s) !== -1).length === sequence.length) {
