  ...
  * onCreate ({ props, context }) {
    ...
    if (!instanceRef) {
      ...
      return yield context.setUrl(
        `/playlist/${playlistRef}/instance/${key}/${current}`
      )
    }
  },
  ...
  render ({ props, state, context }) {
    ...
    return <Playlist {...props} uid={context.uid} />
  }
})
      ...
      if (!prev.state.ready && !playlist.loading) {
        ...
        if (!playlist.value.savedChallenges || newChallenges.length > 0) {
          yield context.firebaseUpdate(
            ...
            newChallenges.reduce(
              (acc, ref, i) => ({
                ...acc,
                [ref]: `${instanceRef}${saved.length + i}`
              }),
              {}
            )
          )
...
const View = fire(props => ({
  ...
  saved: `/saved/${props.playlist.value.savedChallenges[
    props.sequence[props.current]
  ]}`
}))(
  component({
    ...
    controller: {
      * onGameComplete ({ props, context, actions }, data) {
        ...
        yield context.firebaseSet(
          savePath + `/challengeScores/${sequence[current]}`,
          score
        )
      ...
      * awardBadges ({ props, context, actions }, data) {
        ...
        const isNewCompleted =
          completedChallenges.indexOf(sequence[current]) === -1
      ...
      * awardBadge ({ props, context }, badge, isNew) {
        const savedChallenges = props.playlist.value.savedChallenges
        const saveRef = `/saved/${savedChallenges[
          props.sequence[props.current]
        ]}`
        yield context.firebaseUpdate(`/${saveRef}/badges`, { [badge]: 1 })
