...
export default fire((props) => ({
  ...
  render ({props, state, actions}) {
	  ...
	  return (
	    <View savePath={state.savePath} current={current} {...props} listRef={props.ref} {...activeList} next={actions.next} prev={actions.prev} />
	  )
  },
  ...
  controller: {
  	* next ({context, props, state}) {
      ...
      const {username} = context
        ...
        // XXX change this to home in the next version
...
const View = component({
  ...
  controller: {
    * onGameComplete ({props, context}) {
      ...
      const {sequence, current, savePath} = props
      const {uid} = context
      yield context.firebaseTransaction(savePath + '/completed', (val) => (
        (val || []).concat(sequence[current])
      ))
      yield props.next()
