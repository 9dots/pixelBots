...
export default fire(props => ({
  playlist: `/playlistsByUser/${props.uid}/byPlaylistRef/${props.playlistRef}`
}))(
  component({
    render ({ props, state, context }) {
      if (!state.instanceRef) return <span />
      console.log(state.instanceRef)
      // return <span />
      return (
        <Playlist
          {...props}
    ...
    * onUpdate (prev, { props, context, actions }) {
      if (prev.props.playlist.loading && !props.playlist.loading) {
...
const Playlist = fire(props => ({
  playlist: {
    log: console.log(props.instanceRef),
  ...
  component({
    ...
    * onCreate ({ props, state, actions, context }) {
    ...
    render ({ props, state, actions }) {
      ...
      console.log(
        orderedSequence,
        savedChallenges,
        orderedSequence[current].gameRef,
        savedChallenges[orderedSequence[current].key]
      )
    ...
    * onUpdate (prev, { props, state, actions, context }) {
      ...
      if (!prev.state.ready && !playlist.loading) {
        ...
        console.log(sequence, playlist.value)
        ...
        const newChallenges = filter(
          game => saved.indexOf(game.gameRef) === -1,
          sequence
        )
  ...
  component({
    ...
    controller: {
      * onGameComplete ({ props, context, actions }, data) {
        const { sequence, game, savePath, playlist, gameRef } = props
        const { completedChallenges = [] } = playlist.value
        const { uid } = context
        const completed = union(completedChallenges, [gameRef])
