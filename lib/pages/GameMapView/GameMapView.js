...
const router = enroute({
  results: (params, props) => <Results {...params} {...props} type='game' />,
  default: (params, props) => <GameLoader {...params} {...props} type='game' />
})
      ...
      if (!prev.state.ready && !playlist.loading) {
        if (!props.current) {
          return yield context.setUrl(
            `${context.url}/${playlist.value.current}`
          )
        }
    ...
    controller: {
      * next ({ context, props, state }) {
        ...
        if (cur + 1 < orderedSequence.length) {
          yield context.setUrl(`/map/${instanceRef}/play/${cur + 1}`)
        } else {
          yield context.setUrl('/map/view')
        }
      },
      ...
      const titleActions = (onComplete, running, teacherBotRunning) => (
        <Block align='start center'>
          ...
          <Button
            ...
            mr='0.5em'
            h={38}
            onClick={context.setUrl(`/map/view`)}>
            BACK
          </Button>
          <Button
            mr='0.5em'
    ...
    controller: {
      ...
      * awardBadges ({ props, context, actions }, data) {
        ...
        function * maybeAwardBadge (name, comp) {
          if ((stretch.type === name || type === 'read')) {
            yield context.firebaseUpdate(
              savePath + `/challengeScores/${gameRef}`,
              { badge: comp ? 1 : null, available: 2 }
            )
            if (comp) {
...
//     '/activity/:playlistRef/instance/:instanceRef/:current'
//   )
//   const path2Re = toRegexp('/activity/:playlistRef/:current')
//   const hasInstanceRef = path1Re.test(str)

//   if (hasInstanceRef) {
//     const [, playlistRef, instanceRef, current] = path1Re.exec(str)
//     return current
//   }
//   const [, playlistRef, current] = path2Re.exec(str)
//   return current
// }
