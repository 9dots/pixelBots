...
export default component({
	initialState ({props}) {
		...
		if (Object.keys(initialData).length > 0) {
			return {
				...initialGameState,
				...initialData,
				animals: initialData.advanced && initialData.solution
		...
		return (
			game && game.type === 'read'
				...
				: <WritingChallenge
						...
						gameActions={actions}
						savedGame={savedGame}
						docs={createDocs(traits, state.active)}
						gameRef={gameRef}
						saveRef={saveRef}
						playlist={playlist}>
						{children}
	...
	* onRemove ({props, state}) {
	...
	controller: {
		...
		* createRunners ({actions, props}, animals) {
			try {
				const {traits} = props
				return yield animals.map(animal => createIteratorQ(getIterator(animal.sequence, createApi(traits, active))))
			} catch (e) {
