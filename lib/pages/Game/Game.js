  ...
  * onUpdate (prev, {state, props, actions}) {
  ...
  reducer: {
  	...
  	gameDidInitialize: (state, game) => ({ready: true, initialData: game, ...game, speed: state.speed}),
		...
		setCode: (state, id, code) => ({
			...updateAnimal(state, 'sequence', id, code)
		}),
		...
		paste: state => ({
			...updateAnimal(
				...
				'sequence',
				state.active,
				insertAt(state.animals[state.active].sequence || [], state.cursor, state.clipboard.map(b => ({...b})))
			),
			editorState: getEditorState(state)
		}),
...
function getEditorState (state) {
	const sequences = state.animals.map((a) => a.sequence)
	return state.editorState.concat({sequences, cursor: state.cursor})
}
