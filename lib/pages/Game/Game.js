...
const Game = fire((props) => ({
  ...
  * onUpdate (prev, {state, props, actions}) {
  ...
  reducer: {
		...
		animalPaint: (state, {id, location, color}) => ({painted: {...state.painted, [location]: color}}),
  	...
  	gameDidInitialize: (state, game) => ({ready: true, initialData: game, ...game}),
		...
		addBlock: (state, id, block) => ({
			...
			selectedLine: state.selectedLine + (Array.isArray(block) ? block.length : 1)
		}),
