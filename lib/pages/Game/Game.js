...
const Game = fire((props) => ({
  ...
  * onUpdate (prev, {state, props, actions}) {
  ...
  reducer: {
  	...
  	gameDidInitialize: (state, game) => ({ready: true, test: console.log('initialize', game), initialData: game, ...game}),
		...
		selectBlock: ({selected}, block) => ({
			selected: selected.some(b => b === block)
				? selection.filter(b => b !== block)
				: selection.concat(block)
		}),
