...
import createApi, {createDocs, enableAllCapabilities} from 'animalApis'
import frameReducer, {
  ...
  * onRemove ({props, state}) {
    ...
    * reset ({state, actions, props}) {
      ...
      if (advanced && typeof teacherCode === 'string') {
        ...
        yield sleep(500)
        console.log(enableAllCapabilities())
        const startCode = getIterator(teacherCode, createApi(enableAllCapabilities(), animals.length - 1))
