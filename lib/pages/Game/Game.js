...
function insertBlocks (state, blocks, idx) {
	...
	return updateAnimal(
		...
		'sequence',
		state.active,
		insertAt(state.animals[state.active].sequence || [], idx, blocks.map(cloneBlock))
	)
}
...
function selectBlock (state, block) {
	...
	let repeats = 0
	return {
		selected: range(start, end + 1).map(i => seq[i]).filter(b => {
			if (b.type === 'repeat') {
				repeats++
			}
			if (b.type === 'repeat_end') {
				if (repeats === 0) {
					return false
				} else {
					repeats--
...
function surroundBlocks (state, id, selected, block) {
	...
	for (let i = 1; i < idxs.length; i++) {
		if (idxs[i] > idxs[i-1] + 1) {
			sequence.splice((start + inserted++), 0, cloneBlock(block))
			sequence.splice(idxs[i-1] + 1 + (inserted++), 0, cloneBlock({type: 'repeat_end'}))
			start = idxs[i]
		}
	}

	sequence.splice((start + inserted++), 0, cloneBlock(block))
	sequence.splice(idxs[idxs.length - 1] + 1 + (inserted++), 0, cloneBlock({type: 'repeat_end'}))
