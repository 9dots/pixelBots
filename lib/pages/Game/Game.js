...
export default component({
  ...
  * onCreate ({state, actions, context, props}) {
  ...
  render ({props, state, actions, context, children}) {
    const {savedGame = {}, game, gameRef, isDraft, saveRef, playlist, initialData, saved, userProfile, incrementAttempts, incrementSlowdowns} = props
  ...
  * onUpdate (prev, {props, state, actions}) {
    ...
    if(prev.state.speed > state.speed) {
      yield props.incrementSlowdowns()
		...
		if (prev.state.animals && prev.state.animals.some(({sequence}, i) => sequence !== state.animals[i].sequence) || prev.state.targetPainted !== state.targetPainted) {
      if (!props.isDraft) {
