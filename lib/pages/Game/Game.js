  ...
  * onRemove ({ props, state }) {
  ...
  controller: {
    * runCode ({ state, actions }) {
      ...
      } else if (state.hasRun && !state.completed) {
      ...
      yield initCodeWorker({
        ...
        updateAction: actions.mergeFrame
      })
      yield startCodeRun()
    },
    ...
    * stepForward ({ props, state, actions }) {
      if (!state.hasRun) {
        yield [
          actions.setHasRun(true),
          initCodeWorker({ state, updateAction: actions.mergeFrame })
        ]
      }
      yield stepCode()
    },
    ...
    * controllerMove ({ props, state, actions }, fn, ...args) {
    ...
    mergeFrame ({ actions, state }, { data: { payload, type } }) {
      if (type === 'newState') {
        return actions.setFrame({ ...state.frame, ...payload })
      } else if (type === 'done') {
        return actions.onRunFinish()
