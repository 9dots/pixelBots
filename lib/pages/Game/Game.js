  ...
  * onCreate ({state, actions, context, props}) {
    ...
    if (Object.keys(initialData).length > 0) {
      const draftAnimals = initialData.advanced && initialData.solution
        ...
        : initialData.animals
          .map(a => resetAnimalPos(a, game.levelSize[0], a.initial.location))
    ...
    if (game.showModal) {
      yield context.openModal(() => <BotModal
  ...
  render ({props, state, actions, context, children}) {
    const {
      savedGame = {}, game, gameRef, isDraft, saveRef, playlist,
      initialData, saved, userProfile, next
    } = props
  ...
  * onRemove ({props, state}) {
  ...
  controller: {
    ...
    * runCode ({actions, state, context, props}, type) {
      const {
        initialPainted, animals, running, completed, hasRun, editorIsDirty
      } = state
    ...
    * run ({actions, state}) {
      ...
      yield sleep(100)
      pauseState
        ? yield mapValues(({it, i, returnValue}) => (
            actions.runBot(it, i, returnValue)
          ), pauseState)
    ...
    * stepForward ({props, state, actions}) {
      const {
        hasRun, animals, pauseState, stepping, initialPainted, editorIsDirty
      } = state
      ...
      if (!hasRun || editorIsDirty) {
        ...
        const its = yield mapValues(({it, i, returnValue}) => (
          actions.step(it, i, returnValue)
        ), pauseState)
    ...
    * runBot ({actions, state}, it, i, returnValue) {
      ...
      if (args) {
        if (
          state.speed <= 50 ||
          state.steps % (Math.floor((state.speed - 50) / 7) || 1
        ) === 0) {
          yield sleep(interval)
        }
    ...
    * handleStepAction ({actions, state, props}, action) {
      ...
      const [frame, result] = frameReducer({
        ...state,
        active: teacherBotRunning ? animals.length - 1 : state.active
      }, action.type, args)
      ...
      if (!checkBounds(currentLocation, levelSize)) {
        ...
        yield actions.setFrame({
          ...frame,
          invalid: action.type !== 'pickUp' ? 0 : frame.invalid,
          active: state.active,
          activeLine: lineNum
        })
      }
    ...
    * revealTogglePaints ({state, actions}) {
      ...
      for (let loc in togglePaints) {
        ...
        for (let i = 0; i < 6; i++) {
          yield actions.setPainted('painted', loc, toggleColors[i % 2])
  ...
  reducer: {
    ...
    addTeacherBot: (state) => ({
      animals: maybeAddTeacherBot(state.animals).map(a => (
        a.type === 'teacherBot'
          ? {...a, hidden: false}
          : {...a, hidden: true}
      ))
    ...
    setPauseState: (state, payload) => setProp(
      `pauseState.${payload.i}`,
      state,
      payload
    ),
