...
export default component({
  ...
  * onCreate ({state, actions, context, props}) {
    ...
    if (Object.keys(initialData).length > 0) {
      const draftAnimals = initialData.advanced && initialData.solution
        ...
        : initialData.animals.map(a => resetAnimalPos(a, game.levelSize[0], a.initial.location)).concat(addTeacherBot(game.animals))
  ...
  render ({props, state, actions, context, children}) {
    const {savedGame = {}, game, gameRef, isDraft, saveRef, playlist, initialData, saved, userProfile, next} = props
  ...
  * onUpdate (prev, {props, state, actions}) {
    ...
    if (state.type === 'project' && prev.state.targetPainted !== state.targetPainted && !props.isDraft) {
  ...
  * onRemove ({props, state}) {
  ...
  controller: {
    ...
    * runCode ({actions, state, context, props}, type) {
      const {initialPainted, animals, running, completed, hasRun, editorIsDirty} = state
    ...
    * stepForward ({props, state, actions}) {
      const {hasRun, animals, pauseState, stepping, initialPainted, editorIsDirty} = state
