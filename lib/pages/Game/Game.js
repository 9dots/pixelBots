...
export default component({
  ...
  * onRemove ({ props, state }) {
  ...
  controller: {
    ...
    * runCode ({ actions, state, context, props }, type) {
      ...
      if (running) {
        ...
        yield [
          actions.revealTogglePaints(),
          actions.setRunners(its),
      ...
      if (drawing) {
        if (!stepping) {
          const frames = createFrames(
            { ...state, painted: inDrawLoop ? state.painted : state.initialPainted },
            it,
    ...
    * reset ({ state, actions, props }) {
      ...
      const newState = {
        ...
        animals: toggleActiveBot(state, false).map(animal => ({
          ...animal,
  ...
  reducer: {
    ...
    resetPaintedToInitial: (state) => ({painted: state.initialPainted}),
    setInDrawLoop: (state, inDrawLoop) => ({inDrawLoop}),
