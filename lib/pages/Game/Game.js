...
export default component({
  ...
  * onRemove ({props, state}) {
    ...
    * runCode ({actions, state, context, props}, type) {
      ...
      if (running) {
        ...
        yield actions.resetBoard({
          ...
          painted: map(val => val === 'toggle' ? rand(2, 0) > 1 ? 'blue' : 'yellow' : val, initialPainted)
        })
        yield sleep(100)
        if (its) {
    ...
    * runCheck ({actions, state}) {
      ...
      yield actions.reset()
      yield actions.addTeacherBot()
      yield actions.setInitialPainted(
        map(val => val === 'toggle' ? rand(2, 0) > 1 ? 'blue' : 'yellow' : val, initialPainted)
      )
      yield sleep(500)
      const startCode = getIterator(
