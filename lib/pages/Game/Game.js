...
export default component({
  ...
  * onCreate ({state, actions, context, props}) {
		...
		return yield actions.gameDidInitialize({
			...
			solutionIterator: gameState.advanced
				? getIterator(gameState.solution[0].sequence, animalApis[gameState.solution[0].type].default(0))
				: null,
  ...
  * onRemove ({props, state}) {
  ...
  controller: {
    ...
    * handleStepAction ({actions, state}, action) {
      const {payload, type} = action
      const lineNum = payload && payload[payload.length - 1]

      if (!isNaN(lineNum) && lineNum >= 0) {
        yield actions.setActiveLine(lineNum)
      }

      return yield actions[type](...payload)
    },
    ...
    * createRand ({state}, ...args) {
      return createRand(state.rand, ...args)
    },
    * startOver ({state, actions}) {
      yield actions.gameDidInitialize(state.initialData)
    },
