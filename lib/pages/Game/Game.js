...
const GameLoader = fire((props) => ({
  ...
  initialState ({props}) {
    ...
    return {
      ready: false,
      isDraft: Object.keys(initialData).length > 0
    }
  },
  ...
  render ({props, state, actions, children}) {
    ...
    const type = (game.value && game.value.type) || initialData.type
  ...
  * onUpdate (prev, {props, state, actions}) {
  ...
  controller: {
    * onComplete ({props, state, context}) {
      if (!state.isDraft) {
        const {saveRef, gameRef, playlist, onGameComplete} = props
        const {uid} = context
        ...
        yield [
          context.firebaseUpdate(`/saved/${saveRef}`, {
            completed: true
          }),
          context.firebaseUpdate(`/users/${uid}/completed/${saveRef}`, {
            saveRef,
            gameRef,
            linkRef,
            lastEdited: Date.now()
          }),
          context.firebaseUpdate(`/users/${uid}/completedByGame`, {
            [gameRef]: saveRef
          }),
          context.firebaseUpdate(`/links/${linkRef}`, {
            type: 'shared',
            'payload/gameRef': gameRef,
            'payload/saveRef': saveRef
          }),
          context.firebaseSet(`/users/${uid}/inProgress/${gameRef}`, null)
        ]
      }
    },
