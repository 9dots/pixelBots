	...
	* onUpdate (prev, {props, state, actions}) {
	...
	controller: {
		* createRunners ({actions, props, state, context}, animals, onError) {
			try {
				const res = yield animals.map((animal, id) => actions.createIterator(animal, id))
				return res
			} catch (e) {
      ...
      yield context.openModal({
		...
		* createIterator ({context, state}, animal, id) {
		  ...
		  const {value} = yield context.fetch('https://us-central1-artbot-dev.cloudfunctions.net/autoYield', {
		    method: 'POST',
		    body: {
		      code: sequenceToCode(animal.sequence),
		      api: Object.keys(api)
		    }
		  })
		  if (value.name) {
	      return Promise.reject({message: value.name, lineNum: (value.loc.line) - 1})
	    }
	    return Promise.resolve(getIterator(value, api))
		}
	},
...
export default component({
	...
	* onCreate ({props, context, actions, state}) {
	...
	render ({props, state, children}) {
		...
		return <GameLoader key={'game-loader-' + saveRef} {...props} {...state}>
			{children}
		</GameLoader>
	},
	controller: {
