		...
		return (
			game && game.type === 'read'
				?	<ReadingChallenge
						{...actions}
						{...state}
						docs={createDocs(game.capabilities)}
						isDraft={isDraft}
				...
				: <WritingChallenge
						...
						gameActions={actions}
						savedGame={savedGame}
						docs={createDocs(game.capabilities, state.active)}
						gameRef={gameRef}
						saveRef={saveRef}
						playlist={playlist}>
						{children}
		...
		* createRunners ({actions, props, state}, animals) {
			try {
				...
				return yield animals.map(animal => {
					const iter = getIterator(animal.sequence, createApi(game.capabilities, active))
					console.log('iter', iter)
					return createIteratorQ(iter)
				})
			} catch (e) {
