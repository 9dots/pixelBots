...
export default component({
  ...
  * onCreate ({state, actions, context, props}) {
  ...
  render ({props, state, actions, context, children}) {
    const {savedGame, game, gameRef, isDraft, saveRef, playlist, initialData, saved, userProfile} = props
  ...
  * onRemove ({props, state}) {
    ...
    * throwError ({actions, context}, e) {
      ...
      const errorLine = typeof lineNum === 'number'
        ? lineNum
        : stackTrace.parse(e.e ? e.e : e)[0].lineNumber - 6
    ...
    * handleStepAction ({actions, state, props}, action) {
      ...
      try {
        ...
        if (!checkBounds(frame.animals[frame.active].current.location, state.levelSize)) {
          ...
          return result
        } else if (paintedToArray(frame).some(loc => !checkBounds(loc, state.levelSize))) {
          yield actions.throwError({message: 'Paint is out of bounds', lineNum: lineNum - 1 })
          return result
        } else {
        ...
        yield actions.throwError({message: e.message, lineNum: lineNum - 1})
        return
      }
    },
    ...
    * setSpeed ({state, actions, props}, speed) {
      if (speed < state.speed) {
        yield props.incrementSlowdowns()
      }

      return yield actions.setNewSpeed(speed)
    }
  },
...
function paintedToArray (frame) {
  return mapValues((color, loc) => (
    loc.split(',').map(l => Number(l))
  ), frame.painted)
}
