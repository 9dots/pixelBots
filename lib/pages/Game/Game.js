...
import { resetAnimalPos, getAnimalSequences } from 'utils/animal'
...
export default component({
  ...
  * onCreate ({ state, actions, context, props }) {
  ...
  render ({ props, state, actions, context, children }) {
    ...
    const userAnimal = isAnonymous || !userProfile ? 'penguin' : userProfile.bot
      ...
      case 'sandbox':
        return (
          <WritingChallenge
            ...
            savedGame={savedGame}
            playlist={playlist}
            imageUrl={playlist.img || passedState.imageUrl}
            game={game.value}
            gameRef={gameRef}
            saveRef={saveRef}
            isDraft={isDraft}
  ...
  * onRemove ({ props, state }) {
  ...
  controller: {
    ...
    * stepForward ({ props, state, actions }, shouldContinue) {
    ...
    * reset ({ state, actions, props }) {
      ...
      const newState = {
        ...
        runners: {},
        steps: 0,
        cursorPosition: state.cursorPosition || { row: 0, column: 0 },
        infoMessage: null,
        errorLog: null
      }
  ...
  reducer: {
    ...
    replaceGameState: (state, newState) => newState,

    setCursorPosition: (state, newrow, newcolumn) => ({
      ...state,
      cursorPosition: {
