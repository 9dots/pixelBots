  ...
  * onRemove ({ props, state }) {
  ...
  controller: {
    ...
    * step ({ state, actions }, it, i, returnValue) {
      ...
      if (state.drawing && !state.stepping) {
        const frames = createFrames(
          { ...state, painted: state.initialPainted },
          it,
          action => action.type === 'END_DRAW_LOOP'
        )
        return yield actions.setFrame(
          value,
      ...
      yield actions.setFrame(value, frame, it, i, newReturnValue)
    },
    * handleStepAction ({ actions, state, props }, action) {
      ...
      // } else if (action.type === 'drawLoop') {
      //   const [lineNum, fn] = action.payload
      //   const frames = createFrames(
      //     { ...state, painted: state.initialPainted },
      //     fn,
      //     null
      //   )
      //   return [{ ...frames[frames.length - 1], activeLine: lineNum }]
      // }
    ...
    * setFrameValue ({ actions, state }, value, frame, it, i, newReturnValue) {
    ...
    * runCheck ({ actions, state }) {
      ...
      yield actions.runTeacherBot(it, animals.length - 1)
      yield actions.setEditorIsDirty(true)
      yield actions.setTeachRunning(false)
    },
  ...
  reducer: {
    ...
    resetPainted: state => ({ painted: state.initialPainted })
  }
})
