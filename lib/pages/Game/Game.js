...
import createApi, {capabilities, createDocs} from 'animalApis'
...
export default component({
  ...
  * onCreate ({state, actions, context, props}) {
		...
		if (Object.keys(initialData).length > 0) {
			const draftAnimals = initialData.advanced && initialData.solution
				? initialData.solution.concat(teacherBot(game.levelSize))
				: initialData.animals.map(a => resetAnimalPos(a, game.levelSize[0])).concat(teacherBot(game.levelSize))
		...
		const animals = gameState.animals.some(a => a.type === 'teacherBot')
			? gameState.animals
			: gameState.animals.concat(teacherBot(gameState.levelSize))
		...
		return yield actions.gameDidInitialize({
			...
			solutionIterator: gameState.advanced
				? getIterator(gameState.solution[0].sequence, createApi(game.capabilities, gameState.active))
				: null,
  ...
  * onUpdate (prev, {props, state, actions}) {
		...
		if (!props.isDraft && prev.state.animals && prev.state.animals.some(({sequence}, i) => sequence !== state.animals[i].sequence)) {
  ...
  * onRemove ({props, state}) {
    ...
    * reset ({state, actions, props}) {
      ...
      if (advanced && typeof teacherCode === 'string') {
        ...
        const startCode = getIterator(
          teacherCode,
          createApi(capabilities, animals.length - 1)
        )
  ...
  reducer: {
    ...
    removeTeacherBot: (state) => ({
      animals: state.animals.map(a => {
        return a.type === 'teacherBot'
					? {...a, hidden: true, current: a.initial}
					: {...a, hidden: false}
      })
    }),
