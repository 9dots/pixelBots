...
export default fire((props) => ({
  userProfile: `/users/${props.uid}#bindAs=object`
}))(
  component({
    initialState: {
      ready: false
    ...
    render ({props, context, state}) {
      ...
      return (
        <Block tall relative>
          <iframe hidden src='http://192.168.1.38:1337/sso/auth?apiKey=adslfkjasdlfjaldfjalsdfjal1fjlasdfj'/>
          <Block>
          ...
          <Block tall>
            {
              state.ready && context.uid
                ? <Router {...props} userProfile={userProfile.value || undefined} {...state} />
                : <Loading />
            }
          </Block>
        </Block>
