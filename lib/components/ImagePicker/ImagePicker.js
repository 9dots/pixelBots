  ...
  render ({props, context}) {
    ...
    return (
      <Modal w={460} onDismiss={closeModal()}>
        <ModalHeader fs='s' py='l' fontFamily='pixel'>Select Your Bot!</ModalHeader>
        <ModalBody px={40}>
...
const Bot = wrap(CSSContainer, {
	...
	render ({props, context, actions}) {
		...
		return (
			<Block opacity={hovering ? .5 : 1} transition='all .25s' pointer onClick={[actions.setBot, context.closeModal]} >
