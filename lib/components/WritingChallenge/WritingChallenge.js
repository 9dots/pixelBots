...
export default component({
  ...
  render ({props, actions, children, context}) {
    const {playlist, animals, active, initialData, isDraft, type, savedGame, stretch = {}, running, gameHeight = '505px'} = props
    ...
    return (
      <Block tall>
        ...
        <Layout
          ...
          titleActions={playlist && playlist.actions(onComplete, props.running, teacherBotRunning)}>
          {gameDisplay}
        </Layout>
      </Block>
    )
  ...
  controller: {
    * onComplete ({props, context, actions}) {
      ...
      if (stretch && (stretch.hard && props[indicator] > value)) {
        ...
        yield gameActions.setHasRun(true)
        ...
        try {
          ...
          yield context.openModal(() => <CheckingModal savedGame={props.savedGame} onComplete={props.onComplete} next={props.next} correct={status === 'success'}/>)
        } catch (e) {
          console.log()
        }
      }
    ...
    * handleCheckSolutionError ({props, context}) {
      ...
      yield context.toast('Error connecting to solution checker. Please try again.')
    },
    // Hit the cloud functions to check the results of the code
    * checkSolution ({context, props}) {
      const {initialData} = props
      try {
        const res = yield context.fetch(`${process.env.CLOUD_FUNCTIONS}/solutionChecker`, {
          method: 'POST',
