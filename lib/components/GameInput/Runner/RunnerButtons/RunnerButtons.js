...
export default component({
  render ({props, actions, context}) {
  	const {inputType, gameActions, invertSelection, selected, sequence, removeSelected, canAutoComplete, saveRef} = props
    ...
    return (
        ...
        <Checkbox checked={selected.length && selected.length === sequence.length} indeterminate={selected.length && selected.length !== sequence.length} onChange={invertSelection} />
        {
          selected.length > 0 && <RunnerButton
            icon='delete'
            text='Remove blocks'
            bgColor='#666'
