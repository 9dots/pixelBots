...
export default component({
  ...
  * onUpdate (prev, next) {
    const {active, animals, startCode = '', activeLine} = next.props
    const sequence = (animals[active].sequence || '').length < 1
      ? ''
      : animals[active].sequence
    const prevSequence = prev.props.animals[active] && prev.props.animals[active].sequence
    if (prevSequence !== sequence && sequence === startCode && setValue) {
      yield setValue(startCode)
    }

    if (scrollToLine && prev.props.activeLine !== activeLine && !isNaN(activeLine)) {
  	  yield scrollToLine(activeLine - 1)
    }
  }
})
