        ...
        if (sequence || startCode) {
          ...
          newsequence[cursorPosition.row] =
            (editLine.slice(0, cursorPosition.column) +
              (info.argsSnippet ? (info.args.length > 0 ? info.argsSnippet : info.snippet) : info.snippet) +
              editLine.slice(cursorPosition.column) +
              '\n')
          const editorCode = newsequence.join('\n')
          const formattedCode = (key === 'lineBreak' ? editorCode : beautify.js_beautify(editorCode, { indent_size: 2 }) + '\n')
          state.editor.setValue(formattedCode, 1)

        } else {
          state.editor.setValue((info.argsSnippet ? (info.args.length > 0 ? info.argsSnippet : info.snippet) : info.snippet) + '\n')
        }
        yield state.editor.focus()
