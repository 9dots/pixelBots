      ...
      <Block column tall>
        ...
        <Block flex tall wide align='start start'>
          <Block wide tall align='center center'>
            {props.hideApi || (
              <CodeButtons
                log={console.log(docs)}
                readOnly={readOnly}
                docs={getFunctionButtons(docs, sequence, startCode)}
                {...actions}
                w='205px'
  ...
  if (sequence || startCode) {
    ...
    AST.program.body.forEach(element => {
      if (element.type === "FunctionDeclaration") {
        ...
        console.log(element)
        newDocs[functionName] = {snippet: functionName +'()', args: [], description: "Custom function", type:'functions'}
        console.log(newDocs)
      }
    })
  }
