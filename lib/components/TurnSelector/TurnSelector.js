    ...
    return (
    	<Dropdown p='s' btn={btn} left>
    		<Block align='start center' flexWrap='wrap' minWidth={(size + (pad * 2)) * 3}>
    			<Block absolute top bottom left right m='auto' border='1px solid divider' circle={`calc(100% - ${size + (pad * 2)}px)`} />
    			{
    				rose.map((dir) => 
...
const Item = component({
	render ({props}) {
		...
		if(rot !== null)
			return (
				<MenuItem p={pad} borderRadius='100%' onClick={onClick(rot * 90)}>
					<Block 
