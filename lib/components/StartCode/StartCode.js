...
import {filterPaints, createFrames, updateAnimal, getLastFrame} from 'utils/frameReducer'
...
import {createDocs} from 'animalApis'
...
export default component({
  ...
  * onRemove ({actions, props, context, state}) {
    const {animals, capabilities, active} = state
    const it = getIterator(animals[active].sequence, createDocs(capabilities, active))
  	yield props.save({
...
function generateStartGrid (state) {
	...
	const it = getIterator(animal.sequence, animalApis[animal.type].default(0))
	return getLastFrame(state, it)
}
