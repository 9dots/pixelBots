...
export default component({
  ...
  * onCreate ({state, actions}) {
  ...
  render ({props, state, actions}) {
    const {inputType, advanced, userAnimal, type} = props
    const {
      initialData, animals, painted, levelSize,
      active, docs
    } = state
          ...
          <Block tall align='start' wide>
            {
              !props.hideGrid && <Block mr>
                <GridBlock
                  ...
                  id='initial'
                  animals={animals}
                  gridState={{...props.initial, mode: advanced ? null : props.initial.mode}}
                  painted={painted} />
              </Block>
            }
            <GameEditor
  ...
  * onUpdate (prev, {props, state, actions}) {
    ...
    if (props.initialCanvas && prev.state.painted !== state.painted) {
      ...
      newKeys.forEach(key => {
        ...
        props.initialCanvas.updateShapeColor(place, state.painted[key] || 'white')
      })
      if (!props.advanced) {
        yield actions.save()
      }
  ...
  reducer: {
    ...
    updateAnimals: (state, animals) => ({
      animals: animals.map((a, i) => ({...a, sequence: state.animals[i].sequence}))
    }),
