...
export default component({
  ...
  onCreate ({ actions }) {
    return [actions.initializeApp(), actions.initializeMedia()]
  },
  ...
  * onUpdate (prev, next) {
  ...
  render ({ props, state }) {
    return state.uid ? <App key={state.uid} {...state} /> : <span />
  },

  middleware: {
    browser: [locationMw(), trackingMw],
    shared: [
      fetchEncodeJSON,
      fetchMw,
      mediaMw,
  ...
  controller: {
    ...
    * initializeMedia ({ actions }) {
    ...
    firebaseTask: wrapEffect((state, obj) =>
      firebasePush('/queue/tasks', { _state: state, ...obj })
    ),
  ...
  reducer: {
    ...
    setConnected: (state, connected) => ({ connected }),
