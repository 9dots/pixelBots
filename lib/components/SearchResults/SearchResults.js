...
const router = enroute({
  games: (params, { items }) => <ChallengeFeed search games={items} />,
  playlists: (params, { items }) => (
    <PlaylistFeed shouldSort={false} playlists={items} />
  ),
  users: (params, { items }) => <UserFeed users={items} />
})
      ...
      if (!searchQ) {
        return (
          <EmptyState
            ...
            icon='search'
            description={
              'Search to find new challenges and playlists. ' +
          ...
          {loading && searchKey ? (
            ...
            <EmptyState
              ...
              description={`We couldn't find any ${
                tab === 'games' ? 'challenges' : tab
              } for your search "${
                searchQ
              }".  Try simplifying your query or try looking for something different.`} />
          )}
        </Block>
      )
    }
