...
const router = enroute({
  'games': (params, {items}) => (
    <ChallengeFeed mt search games={items} />
  ),
...
export default fire((props) => ({
  ...
  render ({props, context}) {
	  ...
	  if(!searchQ) return <EmptyState title='Find New Challenges' icon='search' description='Search to find new challenges and playlists. Discover new and exciting ways to use PixelBots!' />
...
const Tabs = component({
	render ({props, context}) {
		...
		return (
			<Flex borderBottom='1px solid #e0e0e0' wide relative bottom='0' color='lightBlue' h='42px' align='center'>
		    <Tab
