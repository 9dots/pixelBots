...
export default component({
    ...
    return <CreateLayout {...props} {...paintState} step='create'>
      {
        Switch({
        	...
        	read: () => (
            <StartCode actions={actions} save={props.updateGame} {...props} {...state}>
              {children}
            </StartCode>
          ),
        	debug: () => (
  ...
  * onUpdate (prev, {state, props}) {
    if (prev.state.target.painted && prev.state.target.painted !== state.target.painted) {
      const newKeys = diffKeys(prev.state.target.painted, state.target.painted)
      newKeys.forEach(key => {
        const [y, x] = key.split(',').map(Number)
        const place = x + props.levelSize[0] * y
        state.targetCanvas.updateShapeColor(place, state.target.painted[key] || 'white')
      })
    }

    if (prev.state.initial.painted !== state.initial.painted) {
      const newKeys = diffKeys(prev.state.initial.painted, state.initial.painted)
      newKeys.forEach(key => {
        const [y, x] = key.split(',').map(Number)
        const place = x + props.levelSize[0] * y
        state.initialCanvas.updateShapeColor(place, state.initial.painted[key] || 'white')
      })
    }
  },
  reducer: {
