/**
 * Imports
 */

import TurnSelector from 'components/TurnSelector'
import ErrorTracker from 'components/ErrorTracker'
import ColorPicker from 'components/ColorPicker'
import PaintButton from 'components/PaintButton'
import ReadWidget from 'components/ReadWidget'
import RunWidget from 'components/RunWidget'
import OpacitySlider from './OpacitySlider'
import {component, element} from 'vdux'
import {Block} from 'vdux-containers'
import Grid from 'components/Grid'
import {Icon} from 'vdux-ui'
import Tabs from 'components/Tabs'

/**
 * <Game Output/>
 */


export default component({
	initialState ({props}) {
		return {
			paintMode: false,
  		opacity: props.type === 'read' ? '0' : '0.2',
  		paintColor: 'black',
		}
	},
  render ({props, state, actions}) {
	  const {
	    targetPainted,
	    errorMessage,
	    invalidCount,
	    prevAnimals,
	    description,
	    gameActions,
	    permissions,
	    frameNumber,
	    correctness,
	    animalMove,
	    animalTurn,
	    levelSize,
	    completed,
	    readOnly,
	    canPaint,
	    animals,
	    invalid,
	    running,
	    painted,
	    frames,
	    hasRun,
	    active,
	    paint,
	    reset,
	    steps,
	    speed,
	    size,
	    type
	  } = props

	  const {opacity, paintMode, paintColor, view = 'grid'} = state

	  return (
	    <Block mr='0'>
	      <Block onMouseLeave={actions.setPaintMode(false)} relative pr='10px' pt='0'>
	      	<Block border='1px solid divider' borderBottomWidth={0} bgColor='white' px wide align='center stretch' zIndex='11' relative>
	      		<Tabs tabs={['Grid', 'Directions']} onClick={actions.setView} active='grid' borderBottomWidth={0} tabProps={{flex: true}} />
	      	</Block>
	      	<Block relative zIndex='10' hide={view === 'grid'} sq={350} px py='s' bg='white' border='1px solid divider' overflowY='auto' boxShadow='rgba(black, 0.2) 0px 2px 3px -2px inset'>
	      		{description}
	      	</Block>
	        <Block relative zIndex='10' hide={view === 'directions'}>
	          <Block
	            absolute
	            top='0'
	            left='0px'
	            h={size}
	            w={size}
	            pointerEvents={canPaint ? 'none' : 'default'}
	            zIndex='999'
	            opacity={opacity}>
	            <Grid
	              animals={[]}
	              enableColorTips={!canPaint}
	              active={active}
	              painted={targetPainted}
	              speed={speed}
	              levelSize={size}
	              numRows={levelSize[0]}
	              numColumns={levelSize[1]} />
	          </Block>
	          <Block h={size} w={size}>
	            <Grid
	              id='pixel-art'
	              mode='read'
	              onClick={animalMove(active)}
	              prevAnimals={prevAnimals}
	              hasRun={hasRun}
	              animals={animals}
	              running={running}
	              invalid={invalid}
	              active={active}
	              painted={painted}
	              speed={speed}
	              levelSize={size}
	              numRows={levelSize[0]}
	              numColumns={levelSize[1]} />
	          </Block>
          	<Block border='1px solid divider' borderTopWidth={0} bgColor='white' p='10px' py='20px' wide align='space-around center' hide={canPaint}>
		          <OpacitySlider
	              opacity={opacity}
	              permissions={permissions}
	              onChange={actions.setOpacity}/>
	          </Block>
	          {
	          	canPaint &&
		          	<Block align='center center' bgColor='white' border='1px solid divider' mt='0.5em' wide p>
		          		<ColorPicker
			              clickHandler={actions.setFillColor}
			              animalType={animals[active].type}
			             	paintColor={paintColor}
	              		{...btnProps('green')}
	              		borderColor='rgba(black, .1)'
	              		borderRadius='3px 0 0 3px'
	              		borderWidth='0 1px 0 0'
	              		mr={0}
	              		w={60}
	              		swatchSize={30} />
							    <PaintButton
							    	onClick={paint(active, paintColor)}
							    	{...btnProps('green')}
							    	borderRadius='0 3px 3px 0'
							    	flex />
		          		<TurnSelector
		          			clickHandler={actions.animalTurn(active)}
		          			animals={animals}
		          			{...btnProps('#F6F6F6')}
		          			borderColor='#CCC'
	              		borderWidth='1'
		          			color='primary'
		          			w={60}
		          			ml
		          			flex />
	  	          </Block>
  	        }
	        </Block>
          {
          	readOnly ||
			        <RunWidget
				          {...props}
				          steps={steps}
				          running={running}
				          completed={completed}
				          hasRun={hasRun}
				          hasCode={animals[active].sequence && animals[active].sequence.length > 0}
				          canRun={!canPaint}
				          speed={speed} />
	          }
	      </Block>
	      <ErrorTracker invalid={invalidCount} errorMessage={errorMessage} />
	    </Block>
	  )
  },
  controller: {
  	* animalTurn ({props}, active, turn) {
  		const nextTurn = -props.animals[active].current.rot + turn
  		yield props.animalTurn(active, nextTurn)
  	}
  },
  reducer: {
  	setOpacity: (state, opacity) => ({opacity}),
  	setPaintMode: (state, paintMode) => ({paintMode}),
  	setFillColor: (state, paintColor) => ({paintColor}),
  	setView: (state, view) => ({view}) 
  }
})

function btnProps (color) {
	return {
		boxShadow: `0 4px rgba(black, .3), 0 4px rgba(${color}, 1)`,
		borderWidth: 0,
    color: 'white',
  	bgColor: color,
    fs: 'xl',
    w: 100,
    h: 55,
    mb: 4
	}
}

function animalToInitial (animal) {
  return {
    ...animal,
    current: animal.initial
  }
}
