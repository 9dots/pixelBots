...
export default component({
	...
	render ({props, state, actions, context}) {
		...
		const maxCols = Math.max(Math.ceil(inventorySize / 2), 3)
		const invWidth = (166 - ((maxCols - 1) * 6)) / maxCols
			...
			<Block mr='10px'>
				<Block relative pt='0' align='top' height='100%' mb>
					<Block mr={12}  relative w={200}>
						...
						<Block bgColor='white' p pr={10} border='1px solid divider' mt={8} h={147} flex hide={!inventorySize} column>
							<Block align='start center' flexWrap='wrap' flex>
								{
									range(inventorySize).map(function(i) {
										const bg = getProp(`inventory.${i}.payload`,  animals[active].current)
										return (
											<Block sq={invWidth} border='1px solid divider' mr={6} my={3} bgColor={bg}/>
										)
									})
								}
							</Block>
