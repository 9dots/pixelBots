...
export default component({
	...
	render ({props, state, actions, context}) {
		...
		const multiRow = inventorySize > 3
		const maxWidth = multiRow ? 55 : 100
		const maxCols = multiRow ? Math.ceil(inventorySize / 2) : inventorySize
		const invWidth = Math.min((182 / maxCols), maxWidth)

		return (
			<Block mr='10px'>
				<Block relative pt='0' align='top' height='100%' mb>
					<Block mr={12}  relative w={200} column align='start'>
						<Block border='1px solid divider' bg='white' flex column align='stretch center'>
						<Block h={46} minHeight='46px' wide align='space-around center' px='20' borderBottom='1px solid divider'>
							...
							<IconTab icon='grid_on' name='grid' setView={actions.setView} current={view} color='green' hide={type === 'read'}/>
						...
						<Block overflowY='auto' p sq='100%'>
							<Block align='start center' tall column hide={view !== 'read'}>
								{
									<Block align='center center' wide border='1px solid divider' borderBottomWidth='0' p='s'>
										<LineCounts hard={false}
			                limit={3}
			                name='Error'
			                value={invalidCount}
										 	color='black' />
								...
								<Block border='1px solid divider' h={257} wide overflowY='auto' p='s' fs='xs'>
									{
								...
								<Block>
									{Object.keys(initialPainted).length ? <Toggle mt='s' label='Inspect Pixels' onChange={actions.toggleInspectMode} checked={state.inspectMode} /> : null}
									<PixelInspector
										mt='xs'
						...
						<Block bgColor='white' p='s' border='1px solid divider' mt={8} h={147} column align='space-between start' hide={!inventorySize}>
							<Block fs='13px'>Memory:</Block>
							<Block flex align='center center' mx='auto' w={invWidth * maxCols}>
								<Block align='start center' flexWrap='wrap'>
									{
										range(inventorySize).map(function(i) {
											
											const bg = getProp(`inventory.${i}.payload`,  
												animals[active].current)

											return (
												<Block sq={invWidth} p='xs'>
													<Block sq='100%' outline='1px solid #DDD' bgColor={bg}/>
												</Block>
											)
										})
									}
					...
					<Block >
						<Block relative zIndex='10'>
							...
							{
								canPaint &&
									<Block bgColor='white' border='1px solid divider' mt='0.5em' wide py={12} px={10} align='space-between center'>
										{
