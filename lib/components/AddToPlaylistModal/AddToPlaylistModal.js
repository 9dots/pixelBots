...
export default fire(props => ({
  ...
  component({
    ...
    render ({ props, actions, context, state }) {
      ...
      const filteredPlaylists = orderBy(
        filter(playlist => playlist.creatorID === uid, myPlaylists.value || {}),
        ['lastEdited'],
        ['desc']
      )
            ...
            (playlist, key) => (
              <Block
                ...
                <Button
                  ...
                  Move{' '}
                  <Icon name='reply' transform='scaleX(-1)' ml='s' fs='m' />
                </Button>
              </Block>
            ),
            filteredPlaylists || {}
    ...
    controller: {
      * addToPlaylists ({ context, props }, gameID, ref) {
        const snap = yield context.firebaseOnce(`/playlists/${ref}/sequence`)
        const sequence = snap.val()
        const gameExists = findKey(val => val.gameRef === gameID, sequence) > -1
        yield props.onSubmit()
        if (!gameExists) {
          yield context.firebasePush(`/playlists/${ref}/sequence`, {
            gameRef: gameID,
            order: Object.keys(sequence).length
          })
        }
    ...
    reducer: {
      checkBoxClick: (state, ref) => ({
        ...state,
        selected: maybeAddToArray(ref, state.selected)
      })
    }
  })
)
