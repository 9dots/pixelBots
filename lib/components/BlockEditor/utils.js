...
function getUserFns ({ actions, dispatch, getState }) {
  return next => action => {
    ...
    if (action.type === 'setDocs' || action.type === INIT) {
...
function removeBlocks (sequence, block) {
  ...
  if (block.block) {
    sequence = removeBlocks(sequence, findBlockEnd(sequence, block))
  }
...
/**
 * Change restriction of selected blocks
 * @param  {Array}  sequence  the code sequence
 * @param  {Object} block     the blocks to change restriction
 * @param {string}  restriction the kind of restriction to set
 * @return {Array}            the new code sequence
 */

function changeBlockRestriction (sequence, selected, restriction) {
  const endRestrictions = {}
  return sequence.map(block => {
    const b = { ...block }
    if (selected.map(({ id }) => id).includes(b.id)) {
      b.restriction = restriction
      if (b.block) {
        const id = sequence.find(
          blockEnd => blockEnd === findBlockEnd(sequence, block)
        ).id
        endRestrictions[id] = restriction
      }
    } else if (endRestrictions[b.id]) {
      b.restriction = endRestrictions[b.id]
    }
    return b
  })
}
...
/**
 * find block end of a wrapping block
 * @param  {Array}  sequence  the code sequence
 * @param  {Object} block     the block that we need to find its end
 * @return {Object}           the block end
 */

function findBlockEnd (sequence, block) {
  const idx = sequence.indexOf(block)
  let endBlock
  let level = 0
