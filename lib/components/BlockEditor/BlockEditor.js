...
const CodeBlock = wrap(CSSContainer, {
  ...
  * afterRender ({props}) {
    console.log(props.cursor === props.line)
    if (props.isaAtCursor) {
      console.log('render', props.cursors)
    }
    // yield sleep(100)
    // const element = document.getElementById(`code-block-${props.cursor - 1}`)
    // console.log(element)
    // if (element) {
          ...
          }
            {
              isComment
                ...
                : <Block
                    ...
                    <Icon hide={isLoopEnd} bold fs='30px' name={nameToIcon(block.type)} color={nameToColor(block.type)} />
                  </Block>
            }
            {
