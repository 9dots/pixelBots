...
const CodeBlock = wrap(CSSContainer, {
  ...
  * onCreate ({props}) {
  ...
  render ({props}) {
    ...
    const {block, indentation, line, selected, readOnly, hovering, active, docs, numLines, setArgument, select, isAtCursor, ...rest} = props
    const {type} = block
    ...
    return (
      <Block id={`code-block-${line}`} {...rest}>
        ...
        <Block wide align='start' relative {...rest}>
          <LineNumber line={line} selected={selected} select={select} hovering={hovering} unselectable={docs.unselectable || readOnly} numLines={numLines} block={block} />
          {
