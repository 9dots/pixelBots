...
const CodeBlock = wrap(CSSContainer, {
          ...
          <LineNumber line={line} selected={selected} select={select} hovering={hovering} unselectable={docs.unselectable} numLines={numLines} block={block} />
          {
          ...
          }
            {
              isComment
                ...
                : <Block
                    fs='14px'
                    bgColor={isLoop ? '#a01313' : 'buttons'}
                    highlight={active ?  0.35 : 0}
                    boxShadow={active ? '0 0 7px 2px  rgba(white,.7), 0 0px 3px rgba(0,0,0,.7)' : '0 0'}
                    align='center center'
            ...
            {
              (docs.args || []).map((arg, i) => <BlockArgument arg={arg} setArgument={setArgument(block, i)} value={(block.payload || [])[i]} h={height} />)
            }
        </Block>
      </Block>
    )
  ...
  * onUpdate (prev, next) {
    if (!objEqual(prev.props, next.props) && next.props.active && next.props.running) {
      ...
      if (element) {
        yield element.scrollIntoView(false)
      }
    }
  }
}))
