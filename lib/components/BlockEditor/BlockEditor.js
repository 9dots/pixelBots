...
import {capabilities, typeColors} from 'animalApis'
  ...
  render ({props, state, actions}) {
    ...
    const Blocks = ({
      ...
      <Block
        ...
        key='block-holder'
        h={props.virtual.style.height}
        {...props.virtual.style}>
        {
          props.virtual.items.map((block, i) => <CodeBlock
            line={(props.virtual.offset || 0) + i}
            key={block.id}
            block={block}
            running={running}
            readOnly={readOnly}
            ...
            isAtCursor={cursor === (props.virtual.offset || 0) + i}
            isNew={cursor === (props.virtual.offset || 0) + i + 1}
            indentation={indentations[(props.virtual.offset || 0) + i]}
            active={hasRun && activeLine === (props.virtual.offset || 0) + i + 1}
            key={'block-editor-block-' + (props.virtual.offset || 0) + i}
        ...
        <Block flex tall wide align='start start'>
          <Block wide tall align='center center'>
            ...
            <Block
              ...
              tall>
              <Block>
                <VirtualList items={sequence} InnerList={Blocks} opts={virtualListOpts} />
              </Block>
            </Block>
          </Block>
        </Block>
...
const CodeBlock = wrap(CSSContainer, {
  ...
  * onCreate ({props}) {
  ...
  render ({props, actions}) {
    const {block, indentation, line, selected, readOnly, hovering, active, isNew, docs = {}, numLineDigits, setArgument, select, isAtCursor, clearSelection, removeBlock, ...rest} = props
    const {type} = block
    ...
    return (
      <Block id={`code-block-${line}`} class={['code-block', {'active': active, 'new': isNew}]} {...rest}>
        <BlockCursor ml={(indentation + isBlockEnd) * tabSize} hidden={selected.length || !isAtCursor} w={width} h={margin} />
         <Block wide align='start center' relative boxShadow={isSelected ? '0 0 0 9px rgba(white, .2), inset 0 0 0 99px rgba(white, .2)' : ''} {...rest}>
          ...
          }
          {
            isComment
              ?
                <Block ml={5 + (indentation * tabSize)}>
                  <CommentBlock {...props} />
                </Block>
              : <Block
                  fs='14px'
