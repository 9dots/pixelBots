      ...
      if (block.block) {
        if (selected.length) {
          ...
          sequence = insertBlocks(
            sequence, 
            [blockWithID, {type: 'block_end', parentType: block.type}]
            , cursor
          )
          cursor++
        }
...
const FunctionDefinition = component({
  render ({props, actions}) {
    const {blockProps, block, setBlockPayload, addCapability, ...rest} = props
    return (
      <Block {...rest} {...blockProps} color='white' fontFamily='monospace'>
        <Block mx='s' fs='s' bold>{'FN'}</Block>
        <Input
          m='0'
          h='90%'
          fs='s'
          ...
          onKeyUp={decodeValue(actions.handleKeyUp)}
          value={block.payload} />
      </Block>
    )
  },
  middleware: [
    throttle('handleKeyUp', 300)
  ],
  controller: {
    * handleKeyUp ({props}, val) {
      const {block, setBlockPayload, addCapability} = props
      yield [
        setBlockPayload(block, val),
        addCapability(val, {type: 'functions', args: []}, block.payload)
      ]
    }
  }
})
