...
export default component({
  ...
  render ({ props, state, actions }) {
    ...
    const canSetRestrictions = startCode || false
  ...
  * onUpdate (prev, next) {
    ...
    if (
      !deepEqual(prev.state.sequence, next.state.sequence) &&
      next.props.onChange
    ) {
  ...
  reducer: {
    ...
    invertSelection: ({ selected, sequence, startCode}) => ({
      selected: selected.length
        ? []
        : (sequence || []).filter(
          b =>
            b.type !== 'block_end' && 
              (startCode || b.restriction !== 'locked')
        )
    }),
    ...
    changeRestriction: ({ sequence, selected }, restriction) => ({
      selected: [],
      sequence: changeBlockRestriction(sequence, selected, restriction)
    }),
    ...
    selectBlock: ({ sequence, selected, startCode }, block) => {
      ...
      return { selected: selected.filter(b => startCode || b.restriction !== 'locked') }
    },
