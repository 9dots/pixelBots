  ...
  * onUpdate (prev, next) {
  ...
  controller: {
    ...
    * createNewArgument ({ props, state, actions }, block, arg) {
      const payload = (block.payload || []).slice()
      yield props.editCapability(payload[0], payload.concat(arg).slice(1))
      yield actions.setBlockPayload(block, payload.concat(arg))
    },
    ...
    * openParameterModal ({ context, actions }, block, pos, fn) {
      yield context.openModal(() => (
        <ParameterModal
  ...
  middleware: [stateHistory, getVariableScopes],
      ...
      if (block.block) {
        if (selected.length) {
          ...
          sequence = insertBlocks(
            ...
            [
              block,
              { type: 'block_end', parentType: block.type, id: Date.now() }
            ],
            cursor
          )
          cursor++
