...
export default component({
  render ({props, actions}) {
    const {docs, sequence, cursor, activeLine, setCursor, selectBlock, selected, removeBlock, setArgument, setBlockPayload} = props
...
const CodeBlock = component({
  render ({props}) {
    const {block, indentation, line, selected, active, remove, docs, numLines, setArgument, select, isAtCursor, ...rest} = props

    return (
      <Block {...rest}>
        <BlockCursor ml={indentation * tabSize} hidden={!selected} w='250px' h='18px' />
        <Block wide align='start' relative {...rest}>
          <LineNumber line={line} selected={selected} select={select} numLines={numLines} />
            {
              block.type === 'comment'
...
const LineNumber = component({
  render ({props}) {
    const {selected, select, isAtCursor, line, numLines} = props
