...
export default component({
  render ({props, actions}) {
    const {docs, readOnly, running, sequence, cursor, activeLine, setCursor, selectBlock, selected, setArgument, setBlockPayload} = props
    const indentations = computeIndentation(sequence)

    return (
      <Block id='block-editor' onClick={decodeRaw(actions.selectEnd)} overflowY='auto' flex tall bgColor={props.bgColor} minWidth='480px'>
        {
...
const CodeBlock = wrap(CSSContainer, {
  ...
  * onCreate ({props}) {
    let element = document.querySelector('.code-block.active')
  ...
  render ({props}) {
    const {block, indentation, line, selected, readOnly, hovering, active, docs, numLineDigits, setArgument, select, isAtCursor, ...rest} = props
    const {type} = block
    ...
    return (
      <Block class={['code-block', {'active': isAtCursor}]} {...rest}>
        <BlockCursor ml={(indentation + isLoopEnd) * tabSize} hidden={readOnly || selected.length || !isAtCursor} w={width} h={margin} />
        <Block wide align='start' relative {...rest}>
          <LineNumber line={line} selected={selected} select={select} hovering={hovering} unselectable={docs.unselectable} numLineDigits={numLineDigits} block={block} />
          {
...
const LineNumber = component({
  render ({props}) {
    const {selected, select, block, hovering, line, unselectable, numLineDigits} = props
    const offset = numLineDigits * 6
    const checkProps = {line: line, circle: 25, ml: 5, align: 'center center', fs: 'm'}
      ...
      <Block align='center center' fs='22px' color='#666' w='30px' left={`${offset}px`} {...props}>
        {
          !unselectable && (selected.length || hovering)
            ? <Checkbox
                ...
                onClick={stopPropagation}
                btn={Check}
                checked={selected.indexOf(block) !== -1}
                onChange={select} />
            : <Block {...checkProps} >
                {line + 1}
              </Block>
