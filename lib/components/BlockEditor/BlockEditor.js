  ...
  render ({props, actions}) {
    ...
    return (
      <Block onClick={decodeRaw(actions.selectEnd)} overflowY='auto' flex tall bgColor='#A7B4CB' minWidth='480px'>
        ...
        <BlockCursor hidden={cursor !== sequence.length} w={width} h={margin} />
      </Block>
    )
  },
  controller: {
      ...
      <Block align='center center' fs='22px' color='#666' w='30px' left={`${offset}px`} {...props}>
        {
          !unselectable && (selected.length || hovering)
            ? <Checkbox checkProps={{borderColor: 'rgba(black, .2)', borderRadius: 5, sq: 22, ml: 10, fs: 's'}} onClick={stopPropagation} checked={selected.indexOf(block) !== -1} onChange={select} />
            : <Block mt={2}>{line + 1}</Block>
        }
      </Block>
    )
  }
