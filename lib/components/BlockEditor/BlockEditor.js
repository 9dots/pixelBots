    ...
    return (
      <Block id={`code-block-${line}`} class={['code-block', {'active': active, 'new': isAtCursor}]} {...rest}>
        ...
        <Block wide align='start' relative {...rest}>
          <LineNumber line={line} selected={selected} select={select} hovering={!readOnly && hovering} unselectable={docs.unselectable} numLineDigits={numLineDigits} block={block} />
          {
...
const LineNumber = component({
  render ({props}) {
    ...
    const checkProps = {line: line, circle: 25, ml: 5, align: 'center center', fs: 'xs'}
