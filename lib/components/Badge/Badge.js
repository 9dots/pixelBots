...
export default component({
	render ({props, children}) {
		const {size = 140, count = 1, disabledColor = '#BBB', type, hideTitle = false} = props
		...
		return (
			<Block textAlign='center'>
				<Block
					...
					{
						count
							? <Icon userSelect='none' name={icon} fs={size * .5} color={color} />
				...
				<Block fontFamily='"Press Start 2P"' fs='xxs' mt='l' align='center center' hide={hideTitle}>
					<Block>{title}</Block>
					{
						count > 1
