...
import {Box, Block, Checkbox, Menu, Icon, Image, Text} from 'vdux-ui'
...
export default fire((props) => ({
  ...
  saveGame: `/saved/${props.saveRef}/meta`
}))(component({
  initialState: {
    hovering: false
  },
  render ({props, context, actions, state}) {
    ...
    if (game.loading || saveGame.loading) {
        ...
        <ListItem
          ...
          <Block align='start center'>
            <Box align='start center' w='50%' minWidth={300} pr>
              <Block align='start center'>
                <DragHandle
                  ...
                  ref={ref}/>
                <Block mr={20} sq={75} border='1px solid grey' bgImg={game.value.imageUrl || animalImg} backgroundSize='cover' />
              </Block>
              <Block align='space-between center' flex minWidth={0} w={0} wordWrap='break-word'>
                <Block ellipsis>
                  <Block ellipsis bold pb='s'>{item.title}</Block>
                  <Block ellipsis>
            ...
            <Block hide={isComplete === undefined} flex align='end center'>
              <Badge type={stretchType} size={31} hideTitle effects={false} count={count || 0} description={false}  mr />
              <Status bgColor={isComplete ? 'green' : 'grey'} icon='check' />
            </Block>
          </Block>
        </ListItem>
      </Block>
...
const Status = component({
  render: function({props}) {
    ...
    return (
      <Icon name={icon} bold circle={31} fs={19} bgColor={bgColor} color='white' align='center center' />
    )
  }
