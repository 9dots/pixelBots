...
export default component({
  render ({props, context}) {
    ...
    const {badges = {}} = !!value && value
    const {timeElapsed, slowdowns, runs, steps = 0, stepperSteps = 0, loc} = value && value.meta
		        		...
		        		<Block p column align='center stretch' flex>
		        			{
		        				Switch({
				                  ...
				                  <Stat icon='timer' label='Time' value={getDuration(timeElapsed)}/>
		        						</Block>
		        					),
		        					debug: () => (
		        						<Block column align='center' flex>
