...
export default component({
  render ({ props }) {
    const { readOnly, insertCode, docs, ...rest } = props
              ...
              <Block column align='start center' wide>
                ...
                {fnMap[type].map(key => (
                  <CodeButton
                    key={key}
                    disabled={readOnly}
                   insertCode={!readOnly && insertCode(key, docs[key])}
                    name={key}
                    spec={docs[key]} />
                ))}
              </Block>
  ...
  render ({ props, context, actions }) {
    ...
    return (
      <Button
        ...
        <Text fs='15'> <b> {name === 'comment' ?  '\/\/ comment' : name} </b>{fixedArgs === '(color)' ? '(\'color\')' : fixedArgs} </Text>
      </Button>
    )
  }
})
