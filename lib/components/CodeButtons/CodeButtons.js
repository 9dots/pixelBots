...
export default component({
  render({ props }) {
    const { readOnly, insertCode, docs, sequence, startCode, palette, ...rest } = props
...
const CodeButton = component({
  render({ props, context, actions }) {
    ...
    switch (name) {
      ...
      case 'paintZ':
      case 'paintJ':
        (palette && palette.length > 2) ? fixedArgs = '(\'color\')' : fixedArgs = '()'
        break;
      default:
        fixedArgs = `(${args.join(',')})`
    }
    ...
    return (
      <Button
        ...
        <Text fs='15'> <b> {name === 'comment' ? '\/\/ comment' : (name === 'createFunction' ? 
            <Icon fs='l' bolder name='add' /> : (name === 'ifStatement' ? 'if' : name))} </b>
        {fixedArgs} </Text>
      </Button>
    )
  }
