              ...
              <Block column align='start center' wide>
                ...
                {fnMap[type].map(key => (
                  <BlockButton
                    key={key}
                    disabled={readOnly}
                    addBlock={!readOnly && addBlock(createBlock(key, docs, 'hidden'))}
                    name={key}
                    spec={docs[key]} />
                ))}
              </Block>
...
function createBlock (type, docs) {
  switch (type) {
    ...
    case 'right':
    case 'repeat':
      return { ...docs[type], type, payload: [1], restriction: 'none' }
    case 'ifColor':
      return { ...docs[type], type, payload: ['white'], restriction: 'none' }
    case 'createFunction':
      const userFns = Object.keys(docs).filter(
        doc => doc.search(/^f([0-9]*?)$/gm) > -1
      ).length
      return { type: 'userFn', block: true, payload: ['f' + (userFns + 1)], restriction: 'none' }
    default:
      const payload = docs[type].args && docs[type].args.map(arg => arg.default)
      const block = { ...docs[type], type, restriction: 'none' }
