        ...
        <Block fs='s' fontWeight='800' flex textAlign='center'>
          {
            isSelected
              ...
              : <LineCounts
                  limit={stretch.type === 'lineLimit' && stretch.value}
                  name='line'
                  value={lloc} />
          }
        </Block>
        {
          stretch.type === 'modLimit' && <Block fs='s' fontWeight='800' flex textAlign='center'>
            {
              isSelected
                ? <span/>
                : <LineCounts
                    limit={stretch.type === 'modLimit' && stretch.value}
                    name='modification'
                    value={modifications} />
            }
          </Block>
...
const LineCounts = component({
  render ({props}) {
    const {value, limit, name} = props
    return (
      <Block color={limit && value > limit ? 'red' : 'white'}>
        {
          limit
