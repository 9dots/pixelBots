...
export default component({
  render ({ props, context }) {
    ...
    const allLocked = selected.reduce(
      (acc, block) => acc && block.restriction === 'locked',
      true
    )
    const allHidden = selected.reduce(
      (acc, block) => acc && block.restriction === 'hidden',
      true
    )
    const selectable =
    ...
    const restrictionButtons = [
      <BarButton
        ...
        {...btnProps}
        onClick={
          allLocked ? changeRestriction('none') : changeRestriction('locked')
        } />,
      <BarButton
        key='hide'
        disabled={!isSelected}
        ...
        {...btnProps}
        onClick={
          allHidden ? changeRestriction('none') : changeRestriction('hidden')
        } />
    ]
    return (
      <Block
