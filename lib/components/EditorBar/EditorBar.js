    ...
    return (
      <Block
        ...
        <Block fs='s' fontWeight='800' flex textAlign='center'>
          ...
          {
            !isSelected && (stretch.type === 'lineLimit' || stretch.type === 'modLimit') && (
              <LineCounts
                hard={stretch.hard}
                limit={stretch.value}
                name={stretch.type === 'lineLimit' ? 'line' : 'modification'}
...
const LineCounts = component({
  render ({props}) {
    const {value, limit, name, hard} = props
    const overColor = hard ? 'red' : 'yellow'

    return (
      <Block color={limit && value > limit ? overColor : 'white'}>
        {
          limit
