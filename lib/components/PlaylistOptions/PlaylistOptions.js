...
export default component({
  render ({ props, actions }) {
    const { name, shortLink, playable, play, mine, description } = props

    const btn = (
      <Button focusProps={{ highlight: true }} bgColor='white' color='primary'>
        <Icon color='text' ml={-6} mr={6} name='more_vert' />
        More
      </Button>
    )
    ...
    return (
      <Block align='center center'>
        ...
        <Button mr='5px' bgColor='green' onClick={props.setModal(shortLink)}>
        ...
        <Dropdown btn={btn} zIndex='999'>
          <Menu w='150px' column zIndex='999'>
            ...
            <MenuItem fontWeight='300' onClick={actions.copyPlaylist}>
              Copy Playlist
            </MenuItem>
          </Menu>
        </Dropdown>
    ...
    * copyPlaylist ({ context, props }) {
      ...
      if (context.isAnonymous) {
        return yield context.openModal({
          header: 'Sign In',
          body: 'You must be signed in to copy a playlist.'
        })
      }
      yield context.showToast('Copying playlist')
      yield context.fetch(`${process.env.CLOUD_FUNCTIONS}/copyPlaylist`, {
        method: 'POST',
        body: {
          creatorUsername: username,
          creatorID: uid,
          playlistRef: activeKey
        }
      })
