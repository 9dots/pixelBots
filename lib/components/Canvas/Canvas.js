            ...
            <Animal
              ...
              id={i}>
              {
               type === 'read' && animal.type !== 'teacherBot' &&
                <Block> 
                  <TurnButton onClick={turn(animal.current.rot - 90)} size={size} offset={offset} icon='rotate_left' left />
                  <TurnButton onClick={turn(animal.current.rot + 90)} size={size} offset={offset} icon='rotate_right' right />
                </Block>
              } 
            </Animal>
          ))
...
const TurnButton = component({
  render({props}) {
    const {icon, size, offset, left, right, ...rest} = props
    const off = 0 //offset * -4

    return (
      <Button 
        right={right ? off : 'auto' } 
        left={left ? off : 'auto' } 
        opacity={.7}
