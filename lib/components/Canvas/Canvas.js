...
export default component({
  ...
  * afterRender ({state, actions}, node) {
  ...
  render ({props, state, actions}) {
    const {id, h, w, numRows, animals = [], opacity, type, turn, numberGrid = false} = props
    const {svg} = state
  ...
  * onUpdate (prev, {props, context, state, actions, children}) {
    const {numberGrid = false, painted, palette, numColumns, numRows, id, animals, w, h, onClick = () => {}} = props
    const size = Math.floor(w / numRows)
