...
function middleware ({dispatch, actions, getState}) {
	return next => action => {
		if (action.type === initCanvas.type) {
			const {numRows, cellSize, painted, palette, numberGrid} = action.payload
      ...
      forEach((val, key) => {
        ...
        newCanvasState.addShape(new Rect(
          x * cellSize,
          y * cellSize,
          cellSize,
          cellSize,
...
class Rect {
  constructor (x = 0, y = 0, w = 1, h = 1, coords, ctx, fill = 'white', number) {
    this.x = x
    this.y = y
    this.w = w
    this.h = h
  ...
  draw (ctx = this.ctx, rows, name, palette) {
    ...
    if (this.number) {
      ...
      ctx.strokeText(this.number, this.x + (this.w / 2), this.y + (this.h / 2) + 4)
    }
  }
