...
function middleware ({ dispatch, actions, getState }) {
  return next => action => {
    if (action.type === initCanvas.type) {
      const { numRows, cellSize, painted, coordinates } = action.payload
    ...
    if (action.type === updateCanvas.type) {
      const { numRows, cellSize, painted, palette, coordinates } = action.payload
...
export { initCanvas, updateCanvas }
...
class Rect {
  ...
  draw (ctx = this.ctx, rows, name, palette, hideLines, number) {
    ctx.beginPath({
    ...
    if (number) {
      ctx.strokeStyle = '#9b9898'
      if (this.coordinates[0] === 0) {
        ctx.strokeText(rows - this.coordinates[1] - 1,this.x + 2,this.y + this.h - 2)
      }
      else if (this.coordinates[1] === rows - 1 ){
        ctx.strokeText( this.coordinates[0],this.x + 2,this.y + this.h - 2 )
      }
    }
  ...
  updateColor (color, name, location, palette) {
    const fillColor = color === 'toggle' ? `url(#${name}-img1)` : palette[color]
    document
      .getElementById(`${name}-rect-${location}`)
...
class CanvasState {
  ...
  select (location, color) {
    ...
    const element = document.getElementById(
      `${this.name}-rect-${y * this.numRows + x}`
    )
    forEach((val, key) => element.setAttribute(key, val), selectAttributes)
    const nextSibling = element.nextSibling
     const textElement = nextSibling && nextSibling.tagName === 'text'
      ? element.nextSibling
      : null
  	element.parentElement.appendChild(element)
    textElement && element.parentElement.appendChild(textElement)
  }
  clearSelection () {
    if (this.selected) {
      const [prevX, prevY] = this.selected || []
      const element = document.getElementById(
        `${this.name}-rect-${prevY * this.numRows + prevX}`
      )
  ...
  updateShapeColor (location, color) {
    if (this.shapes[location]) {
      this.shapes[location].updateColor(
        color,
        this.name,
        location,
        this.colorPalette
