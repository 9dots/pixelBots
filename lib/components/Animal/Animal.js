...
export default component({
  render ({props, state}) {
    ...
    const speed = animal.type === 'teacherBot' ? 0.1 : animationSpeed

	  return (
	    <Block
	      absolute
	      transition={
	      	(mode !== 'edit' && hasRun || animal.type === 'teacherBot') &&
          `all ${speed < 0.1 ? 0.1 : speed}s ease-in-out, opacity .5s ease-in-out`
	      }
	      ...
	      pointerEvents='none'
	    	{...restProps}>
	    		<Block
	    			...
	    			background={`url(${gameImages[animal.type] || images[animal.type]})`}
