...
import { images, gameImages } from 'animalApis'
...
export default component({
  render ({ props, state, context, children }) {
    ...
    const type =
      animal.type === 'teacherBot' ? animal.type : userAnimal || 'penguin'

    const { rot } = animal.current
    const animalSize = Math.floor(parseFloat(cellSize) / 1.1) + 'px'
    const pos = getPosition()
    const normalizedSpeed = isRead ? 0.25 : getInterval(null, speed) / 1000
  ...
  * onUpdate (prev, next) {
    if (
      prev.props.invalid !== next.props.invalid &&
      next.props.invalid &&
      !next.state.error
    ) {
