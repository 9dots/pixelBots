  ...
  component({
    * onCreate ({ props, actions, context }) {
    ...
    render ({ props, actions }) {
      ...
      const { type } = block
      const restricted = restriction && restriction !== 'none' && !startCode
      ...
      const showIcon = restriction !== 'none' && (startCode || restriction === 'locked')
      const optionalIcon = showIcon && startCode && <Icon name={ restriction === 'locked' ? 'lock' : 'visibility_off'} color ='white' />

      return (
        <Block
          cursor={restricted ? 'not-allowed' : 'pointer'}
          opacity={!startCode && restriction === 'locked' ? 0.6 : 1}
          id={`code-block-${line}`}
          class={['code-block', { active: active, new: isAtCursor || isNew }]}
          {...omit('key', rest)}>
          <BlockCursor
            ml={(indentation + isBlockEnd) * tabSize}
            hidden={readOnly || isSelecting || !isAtCursor || restricted}
            block={block}
            w={width}
            h={margin} />
          <Block
            ...
            <LineNumber
              ...
              unselectable={docs.unselectable || restricted}
...
const BlockBody = component({
  render ({ props, actions }) {
    const { indentation, tabSize, block, active, isBlockEnd, bgColor, optionalIcon, restricted } = props
    const blockProps = {
      ...getBlockProps(active, height, bgColor, block),
    ...
    switch (block.type) {
      case 'comment':
        return <CommentBlock optionalIcon={optionalIcon} color='white' ml={blockProps.ml} {...props} />
      case 'lineBreak':
      ...
      <Block align='start center' pointerEvents={readOnly ? 'none' : 'all'}>
        <Block
          ...
          <Button
            fs='s'
            p='s'
            hidden={!hovering || readOnly || restricted}
            icon='edit'
            opacity={0.6}
            hoverProps={{ opacity: 1 }}
        ...
        {args.map(arg => (
          <Block
            ...
            fontFamily='monospace'
            align='center center'
            onClick={restricted ? null : props.openParameterModal(block, arg)}
            relative
            {...blockProps}
            ml='s'
            minWidth={width}
...
const CommentBlock = component({
  render ({ props }) {
    const { setBlockPayload, block, ml, optionalIcon, ...restProps } = props
