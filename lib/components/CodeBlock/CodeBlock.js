      ...
      return (
        <Block
          ...
          <Block
            ...
            relative
            boxShadow={
              active || (readOnly && isAtCursor)
                ? '0 0 0 9px rgba(white, .3), inset 0 0 0 99px rgba(white, .3)'
                : ''
            }
            {...omit('key', rest)}>
            <LineNumber
            ...
            {restriction === 'locked' && <Icon key={block.id + 'lock'} name='lock' ml='s' fs='m' />}
            {hovering &&
              !readOnly &&
              !restricted &&
              !docs.unselectable && (
  ...
  render ({ props, actions, context }) {
    ...
    return (
      <Block align='start center' pointerEvents={readOnly ? 'none' : 'all'}>
        <Block {...rest} {...blockProps} color='white' fontFamily='monospace'>
