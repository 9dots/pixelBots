...
export default fire(props => ({
  ...
  myProgress: {
    ...
    join: {
      ...
      childRef: (val, ref) =>
        ref.child(val.instanceRef).child('completedChallenges')
    }
  }
}))(
  component({
    render ({ props, context }) {
      ...
      const clickHandler = props.clickHandler
        ? props.clickHandler
        : mine ? context.setUrl(base + '/edit') : context.setUrl(base + '/view')
      ...
      const progValue = myProgress.value
        ? myProgress.value.completedChallenges
        : []
      const sequence = Object.keys(value.sequence || {}) || []
          ...
          <Block w={270} h={90} ml column align='space-around'>
            <Block wide align='start center'>
              <DetailInfo
                icon='collections'
                pr
                label={`${sequence ? sequence.length : 0} challenges`} />
              <DetailInfo
                icon='person'
                pr
                onClick={context.setUrl(
                  `/${value.creatorUsername}/authored/playlists`
                )}
