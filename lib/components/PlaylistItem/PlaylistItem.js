...
export default fire((props) => ({
  ...
  myProgress: `/users/${props.uid}/lists/${props.playlistRef}`
}))(component({
  render ({props, context}) {
    ...
    const imageSize = 90
    const {loading: myProgressLoading, value: myProgressValue} = myProgress
    const {loading, value} = playlist

    if (loading || myProgressLoading) return <IndeterminateProgress />
