...
export default fire((props) => ({
  ...
  render ({props, context}) {
    ...
    const imageSize = 50//90
    const {loading, value} = playlist

    if (loading) return <IndeterminateProgress />

    return (
      <ListItem mb={-1} p='1em' wide align='start stretch' onClick={clickHandler}>
        <Block mr='20px' sq={imageSize} overflow='hidden'>
          {
            value.imageUrl
              ? <Image sq={imageSize} src={value.imageUrl} border='1px solid grey' />
              : <Block sq={imageSize} bgColor='blue' align='center center'>
