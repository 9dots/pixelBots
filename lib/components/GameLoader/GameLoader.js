...
const Loader = component({
  render ({ props, state, children }) {
    const { game, initialData, savedGame, isSingleChallenge } = props
  ...
  component({
    ...
    * onCreate ({ actions }) {
    ...
    render ({ props, state, children, actions }) {
      const { initialData, saveRef, saveData: { value = {} } } = props
    ...
    controller: {
      ...
      * onComplete ({ props, state, context, actions }, data = {}) {
        ...
        if (savedGame.externalUpdate) {
          yield context.fetch(
            `${process.env.CLOUD_FUNCTIONS}/externalUpdate`,
            {
              method: 'POST',
              body: {
                progress: 100,
      ...
      * incrementMoves ({ props, context }) {
        yield context.firebaseTransaction(
          `/saved/${props.saveRef}/meta/moves`,
          val => val + 1
        )
      },
