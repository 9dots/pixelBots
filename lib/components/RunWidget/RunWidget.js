...
export default component({
  render ({props, actions}) {
		const {hasRun, speed, running, canRun, steps = 0, completed, gameActions, hasCode, isProject} = props
	  const current = getSymbols(canRun, running)

	  const btnProps = isProject ? {px: 12, fs: 35} : {px: 24, fs: 42}

	  return (
	  	<Block bgColor='white' border='1px solid divider' mt='0.5em' wide p='10px'>
	  		<Block align='center center'>
		  		<GameButton disabled={running} bg='red' mr='s' onClick={gameActions.reset()} py='5px' {...btnProps}>
	  				<Icon fs='inherit' bolder name='replay' />
		  		</GameButton>
		  		<GameButton minWidth={isProject ? 'initial' : 100} disabled={!hasCode} bg='green' mr='s' flex onClick={actions.handleClick()} {...btnProps}>
	  				<Icon ml='-2px' fs='inherit' name={current.icon} />
		  		</GameButton>
		  		<GameButton disabled={running || !hasCode} bg='yellow' onClick={gameActions.stepForward()} py='5px' {...btnProps}>
	  				<Icon fs='inherit' bolder name='skip_next' />
		  		</GameButton>
		  		{
		  			isProject && 
			  			<GameButton disabled={running || !hasCode} bg='blue' onClick={gameActions.onComplete} h={47} fs={17} p='5px 14px' ml='s'>
			  				SUBMIT
		  				</GameButton> 
	  			}
	  		</Block>
