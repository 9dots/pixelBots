	  ...
	  return (
	  	<Block bgColor='white' border='1px solid divider' mt='0.5em' wide p='10px'>
	  		<Block align='center center'>
		  		...
		  		<GameButton disabled={running || completed || !hasCode} bg='yellow' onClick={gameActions.stepForward()} p='5px 24px'>
	  				<Icon fs={42} bolder name='skip_next' />
		  		</GameButton>
		  		{
		  			isProject && <GameButton disabled={running || completed || !hasCode} bg='blue' onClick={gameActions.onComplete} p='5px 24px'>
		  				<Icon fs={42} bolder name='check'/>
	  				</GameButton> 
	  			}
	  		</Block>
