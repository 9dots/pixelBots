	  ...
	  return (
	  	<Block bgColor='white' border='1px solid divider' mt='0.5em' wide p='10px'>
	  		<Block align='center center'>
		  		<GameButton disabled={running || teacherBotRunning} bg='red' mr='s' onClick={gameActions.reset()} py='5px' {...btnProps}>
	  				<Icon fs='inherit' bolder name='replay' />
		  		</GameButton>
		  		<GameButton minWidth={isProject ? 'initial' : 100} disabled={!hasCode || teacherBotRunning} bg='green' mr='s' flex onClick={actions.handleClick()} {...btnProps}>
	  				<Icon ml='-2px' fs='inherit' name={current.icon} />
		  		</GameButton>
		  		<GameButton disabled={completed || running || !hasCode || teacherBotRunning} bg='yellow' onClick={gameActions.stepForward()} py='5px' {...btnProps}>
