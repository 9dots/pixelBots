...
const caseInsensitiveName = game =>
  game.name ? game.name.toLowerCase() : game.title.toLowerCase()
...
export default component({
  ...
  * onUpdate (prev, { props, actions, state }) {
    if (
      prev.state.orderBy !== state.orderBy ||
      prev.state.order !== state.order ||
      (!objEqual(prev.props.games, props.games) && props.games)
    ) {
      const sortedGames = sort(
        map((game, key) => ({ ...game, key }), props.games),
        state.orderBy,
  ...
  render ({ props, actions, context, state }) {
    const { games, mine, search, ...rest } = props
    const { uid } = context
    ...
    return (
      <Block wide margin='0 auto' {...rest}>
        <Block wide display='block' minWidth='820px'>
          <Block
            pl='40px'
            pr='16px'
            py='8px'
  ...
  controller: {
    * showModal ({ context }, link) {
      yield context.openModal(<LinkModal code={link} footer={modalFooter} />)
    },
