...
export default (component({
  ...
  render ({props, context, state, children, actions}) {
    ...
    const drawFrames = buildFrames(state, frames)
    const prevAnimals = drawFrames[frameNumber - 1]
      ? drawFrames[frameNumber - 1].animals
      : game.animals
  ...
  * onUpdate (prev, {state, actions, props}) {
  ...
  reducer: {
    animalPaint: (state, color) => addFrame(
      state,
      paintSquare({id: state.active, color})
    ),
    animalMove: (state, location) => addFrame(
      state,
      moveAnimal({id: state.active, getLocation: curLocation => location})
    ),
    animalTurn: (state, deg = 90) => addFrame(
      state, turnAnimal({
      	id: state.active,
      	turn: deg
      })
