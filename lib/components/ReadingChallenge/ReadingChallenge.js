...
import {validate} from 'utils/frameReducer'
...
export default component({
  render ({props, context, state, children, actions}) {
  	const {game, savedGame, playlist, isDraft, draftData} = props
  	const {invalid, errorMessage} = state
  ...
  * onUpdate (prev, {props, actions}) {
  ...
  controller: {
    * paint ({props, actions}, id, color) {
      const {paints = 0, frames, animals, active, animalPaint} = props
      ...
      const error = validate(
        frames,
        paints,
        animals[active].current.location,
        color
      )
