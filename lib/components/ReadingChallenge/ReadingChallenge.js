...
export default (component({
  initialState ({props}) {
    ...
    return {
      paints: 0,
      ...game,
      animals: savedGame && savedGame.animals 
        ? savedGame.animals.map(a => ({...a, sequence: a.sequence || game.startCode}))
        : game.animals.map(a => ({...a, sequence: a.sequence || game.startCode})),
      correctness: [],
      frameNumber: -1,
      saved: true,
      states: [],
  ...
  render ({props, context, state, children, actions}) {
  	...
  	const {active, invalid, invalidCount, frameNumber, frames, correctness} = state
  ...
  * onUpdate (prev, {state, actions}) {
    if (!deepEqual(prev.state.painted, state.painted) || !deepEqual(prev.state.animals, state.animals)) {
      yield actions.clearError()
      yield actions.save()
    }
  },
  ...
  controller: {
    * save ({context, actions, state, props}) {
      const {paints, painted = {}, animals, invalidCount = 0} = state
