...
export default component({
  initialState ({ props }) {
    const { game, painted, animals, paints, docs, levelSize } = props
    ...
    return {
      ...
      frames: createPaintFrames({ painted, animals, paints, levelSize }, it),
      gameStates: []
    }
  },

  render ({ props, context, state, children, actions }) {
    const { game, savedGame, playlist, isDraft, isSingleChallenge, gameHeight = '50px' } = props
  ...
  * onUpdate (prev, { props, actions, state }) {
  ...
  controller: {
    ...
    * onComplete ({ context, props, state }) {
      ...
      const { isSingleChallenge, stretch, saveRef } = props
    ...
    * undo ({ state, props, actions }) {
      ...
      yield actions.replaceState(gameStates[gameStates.length - 2][0], {
  ...
  reducer: {
    ...
    replaceState: (state, newState, keepErrors) =>
      Object.assign(newState, keepErrors),
