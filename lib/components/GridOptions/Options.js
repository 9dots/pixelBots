...
export default component({
  render ({props, actions}) {
    ...
    const advProps = advanced ? { bgColor: 'blue', color: 'white'} : {}
    const btn = (
    ...
    return (
      <Block align='start center'>
        <Button
        	...
        	{...btnProps}
        	{...advProps}>
          {advanced ? 'Standard' : 'Advanced'} Editor
          <Tooltip hide={advanced} h={20} message='Use the advanced editor to programatically generate start grids.' placement='bottom' tooltipProps={{whiteSpace: 'normal', w: 200, fs: 'xs', textAlign: 'left'}}>
        ...
        </Button>
        {
          advanced
            ? <Block align='start center'>
              <Button {...btnProps} bgColor='white' color='primary' hoverProps={{highlight: 0.02}} focusProps={{highlight: 0.04}}
