...
export default component({
  initialState ({props}) {
    return {
      ...
      startCode: props.type === 'debug' || props.type === 'project'
    }
  },
  render ({props, state, actions, children}) {
  	const {type, animals, levelSize, advanced, target, initial, ...restProps} = props
  	...
  	* setSize ({props, state, context, actions}, size) {
      const {animals} = props
        ...
        animals: animals.map((a) => ({
  		...
  		yield props.actions.setBulkPainted('target', filter((val, key) => (
        key.split(',').every((v) => v <= size - 1)
      ), props.target.painted))

  		yield props.actions.setBulkPainted('initial', filter((val, key) => (
        key.split(',').every((v) => v <= size - 1)
