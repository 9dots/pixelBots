...
const randColor = createAction('randColor', (...args) => args)
const drawLoop = createAction('drawLoop', (...args) => args)
const frameOf = createAction('getModdedFrame', (...args) => args)
const setNumFrames = createAction('setNumFrames', (...args) => args)
...
const stopDraw = createAction('exitDrawLoop', (...args) => args)
...
function * randColorPicker (lineNum, ...colors) {
  return yield randColor(lineNum, colors)
}
...
const capabilities = {
  ...
  stopDraw: {
    type: 'animation',
    description: 'Stops repeating the draw loop when called.',
    snippet: 'stopDraw()'
  },
  block_end: {
    type: null,
  ...
  ifStatement: {
    type: 'control',
    description: 'general if statement',
    snippet: 'if(true) {\n\t\n}',
    wrapSnippet: 'if (true) {',
    block: true
  },
  ...
  setNumFrames: {
    type: 'animation',
    description: 'Sets the number of animation frames to the given number.',
    snippet: 'setNumFrames()',
    argsSnippet: 'setNumFrames(4)',
    defaultArgs: [4],
    args: [
      {
        name: 'limit',
