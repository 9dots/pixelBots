...
export default function (spec, id) {
  return map(createActionFromSpec(id), spec)
}

function createArgumentsFromSpec (id, spec, name) {
  return (lineNum, ...args) => {
    const res = [lineNum, id]

    if (Array.isArray(spec)) {
      return res.concat(spec.map((aspec, i) => {
        aspec = aspec === true
          ? capabilities[name].args[i]
          : aspec

        if (aspec.values && aspec.values.indexOf(args[i]) === -1) {
      ...
      : {
          ...
          args: (val.args || []).map((arg, i) => ({
            ...
            values: (traits[key][i] === true
              ? arg.name === 'color' ? palette : arg.values
              : traits[key][i]) || null
          }))
        },
