        ...
        <Box relative wide color='#767676' fontWeight='800'>
          <Block p='16px 20px' align='start center' flex {...titleProps}>
            {titleImg && (
              <Block relative bgColor='white'>
                <Avatar
                  ...
                  sq='50'
                  src={titleImg}
                  borderRadius={imgRounded ? '9999px' : 0} />
                <Block
            ...
            <Block align='start center' flex relative ml='1em'>
              {navigation
                .filter(nav => !!nav)
                .map(({ category, title, onClick }, i, arr) => (
                  <Block key={i} align='start center'>
                    <Block
                      ...
                      <Text display='block' fontWeight='500' fs='l'>
                        {title}
                        {category === 'Sandbox Mode' && (
                          <Icon fs='m' ml='0.5em' name='edit_mode' />
                        )}
                      </Text>
                    </Block>
                    {i < arr.length - 1 && (
                      <Icon fs='l' mx='1em' name='keyboard_arrow_right' />
                    )}
                  </Block>
                ))}
              {badges.length ? (
