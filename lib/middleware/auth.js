...
export default ({actions, dispatch}) => {
  ...
  function * startWeoSignIn (token) {
    const {value} = yield actions.fetch('https://us-central1-artbot-dev.cloudfunctions.net/weoAuth', {
      method: 'POST',
      body: JSON.stringify({
        token
      })
  ...
  return (next) => (action) => {
    ...
    if (action.type === signInWithWeo.type) {
      dispatch(startWeoSignIn(action.payload))
        .then(({token, displayName, username}) => {
          auth().signInWithCustomToken(token)
            .then(user => user
              .updateProfile({displayName})
              .then(() => user.updateEmail(`${username}@pixelbotuser.io`))
            )
        })
