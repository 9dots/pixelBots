...
test('<frameReducer/> validate tests', t => {
  ...
  //t.notEqual(validate([], 0, location, 'black'), null, 'takes empty frames array')
  t.equal(validate(frames, 1, location, 'red'), 'location', 'returns location error')
  t.equal(validate(frames, 0, location, 'black'), 'color', 'returns color error')
...
test('<frameReducer/> paint tests', t => {
  ...
  t.deepEqual(isPaintedState.painted, {'3,1':'black', '4,0':'red'}, 'adds coord to painted')
  ...
  t.deepEqual(nowIsPaintedState.painted, {'4,0':'black'}, 'adds painted prop to state')
})
...
test('<frameReducer/> updateAnimal tests', t => {
  ...
  const afterRotAnimals = [
    {'current': {'rot': 0}},
    {'current': {'rot': -1}},
  ]

  t.deepEqual(updateAnimal(animals, 'current.rot', 1, -1), afterRotAnimals, 'animal prop gets updated')
})
...
test('<frameReducer/> computeLocation tests', t => {
  ...
  t.deepEqual(computeLocation(animalState, 0, (l, r) => l.map(x => x+1)), [5, 1], 'takes a function')
})
...
test('<frameReducer/> setAnimalPos tests', t => {
  ...
  t.deepEqual(setAnimalPos(startFrame, 1, [1, 1]).animals, movedAnimals, 'updates animal location')
})
...
test('<frameReducer/> addAnimalPos tests', t => {
  ...
  t.deepEqual(up(startFrame, 0, 1).animals[0], movedAnimals[0], 'moves animal up')
  t.deepEqual(down(startFrame, 1, 1).animals[1], movedAnimals[1], 'moves animal down')
  t.deepEqual(left(startFrame, 2, 2).animals[2], movedAnimals[2], 'moves animal left')
  t.deepEqual(right(startFrame, 3, 2).animals[3], movedAnimals[3], 'moves animal right')
})

test('<frameReducer/> turn and forward tests', t => {
  t.plan(3)

  const turnRight = (frame, ...args) => frameReducer(frame, 'turnRight', args)[0]
  ...
  t.deepEqual(turnRight(startFrame, 0).animals[0], rotatedAnimals[0], 'rotates animal right')
  t.deepEqual(turnLeft(startFrame, 1).animals[1], rotatedAnimals[1], 'rotates animal left')
  t.deepEqual(forward(turnRight(startFrame, 2), 2).animals[2], rotatedAnimals[2], 'animal turns then moves forward')
})
...
test('<frameReducer/> getCurrentColor tests', t => {
  ...
  t.equal(getCurrentColor(activeOneFrame)[1], 'white', 'returns white if no color')
})
...
test('<frameReducer/> createRand tests', t => {
  ...
  t.equal(createRand(framesWithRand, -1, 4, 7)[1], 11, 'handles min and max params')
})

test('<frameReducer/> isEqualSequence tests', t => {
  t.plan(3)

  const sequenceA = [{'sequence': 'a,b,c'},{'sequence': '1,2,3'},{'hasNoSequence':'nope'}]
  const sequenceB = [{'sequence': 'd,e,f'},{'sequence': '2,3,4'},{'alsoNoSequence':'nope'}]

  t.ok(isEqualSequence(sequenceA, sequenceA), 'same reference returns true')
  t.ok(isEqualSequence(sequenceB, [{'sequence': 'd,e,f'},{'sequence': '2,3,4'},{}]), 'different reference but deepEqual returns true')
...
test('<frameReducer/> iterator tests', t => {
  ...
  t.equal(typeof iteratorA, 'object', 'generator object gets created from sequence')
...
test('<frameReducer/> createFrames and createPaintFrames tests', t => {
  ...
  const createdFramesA = createFrames(startFrame, getIterator(sequenceA, apiA), 'seed')
  ...
  const paintFramesA = createPaintFrames(startFrame, getIterator(sequenceA, apiA), 'seed')
...
test('<frameReducer/> getLastFrame and getLastTeacherFrame tests', t => {
  ...
  const lastTeacherFrameB = getLastTeacherFrame(startFrame, getIterator(sequenceA, apiA))
  t.notOk(Array.isArray(lastFrameA), 'returns one teacher frame')
  t.deepEqual(lastFrameA, endFrame, 'iterates, paints, returns last teacher frame')
})
