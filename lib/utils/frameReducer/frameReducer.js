...
function right (frame, id, steps = 1) {
  return strafe(frame, id, [0, steps])
}
...
function up (frame, id, steps = 1) {
  return strafe(frame, id, [-steps, 0])
}
...
function strafe (frame, id, vector) {
  return frame.wrappedMovement
    ? setAnimalPos(frame, id, wrappedMove(frame, id, vector))
    : addAnimalPos(frame, id, vector)
}

function wrappedMove (state, id, vector) {
  const [width, height] = state.levelSize
  const { location } = state.animals[id].current
  const [y, x] = vadd(location, vector)
  const pos = width * y + x
  const newY = clamp(Math.floor(pos / width), height - 1)
  const newX = pos < 0 ? x : pos - newY * width

  return [newY, clamp(newX, width - 1)]
}

function clamp (val, max = 10, min = 0) {
...
function pickUp (state, id) {
  ...
  const currentColor =
    state.painted[location] === 'white' ? null : state.painted[location]
  const { inventory = [] } = state.animals[id].current
  if (inventory.length + 1 > state.inventorySize || !currentColor) {
    return {
      ...state,
      invalid: 1
    }
  ...
  return {
    ...state,
    animals: updateAnimal(state.animals, 'current.inventory', id, [
      { type: 'color', payload: currentColor },
      ...(state.animals[id].current.inventory || [])
    ]),
    painted: {
      ...state.painted,
      [location]: null
