...
function pickUp (state, id) {
  ...
  return {
    ...
    animals: updateAnimal(
      ...
      'current.inventory',
      id,
      (state.animals[id].current.inventory || []).concat({
        type: 'color',
...
function place (state, id) {
  ...
  const toPlace = inventory[inventory.length - 1]
  if (!toPlace) return state
  return {
    ...state,
    animals: updateAnimal(state.animals, 'current.inventory', id, inventory.slice(0, -1)),
    painted: {
      ...state.painted,
