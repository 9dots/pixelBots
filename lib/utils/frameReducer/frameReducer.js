...
function createRand (frame, seed = Math.random() * 1000, lineNum, min, max) {
  if (max === undefined) {
    max = min
    min = 0
  }

  return [frame, Math.floor(srand(seed)(min, max))]
}
...
function generatePainted (state, code) {
  const {initialPainted, animals, active, randSeed} = state
