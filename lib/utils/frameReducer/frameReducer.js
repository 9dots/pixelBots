...
const actions = {
  ...
  setFrameRate: updateRunSpeed,
  frameOf: getModdedFrame,
  rand: createRand,
  move: forward,
  stopDraw: exitDrawLoop,
  randColor,
  getCurrentColor,
  startDrawLoop,
  getLoopState,
 ...
 * Actions
 */
function startDrawLoop (frame, id) {
  return { ...frame, drawing: true, inDrawLoop: false, frame: 1 }
}

function exitDrawLoop (frame, id) {
  return {
    ...frame,
    stopDrawing: true,
    drawing: false,
...
function setNumFrames (frame, id, limit) {
  return { ...frame, numFrames: limit }
}

function right (frame, id, steps = 1) {
  return addAnimalPos(frame, id, [0, steps])
}
function left (frame, id, steps = 1) {
  return addAnimalPos(frame, id, [0, -steps])
}
function up (frame, id, steps = 1) {
  return addAnimalPos(frame, id, [-steps, 0])
}
function down (frame, id, steps = 1) {
  return addAnimalPos(frame, id, [steps, 0])
}

function getLoopState (state, id) {
  return [state, state.loops[id]]
}

function addLoop (state, id) {
  return {
    ...state,
    loops: { ...state.loops, [id]: (state.loops[id] || []).concat(true) }
  }
}
...
function removeLoop (state, id) {
  return {
    ...state,
    loops: { ...state.loops, [id]: state.loops[id].slice(0, -1) }
  }
}

function forward (state, id, steps = 1) {
  const { rot } = state.animals[id].current
  return addAnimalPos(state, id, vmul(Number(steps), getHeading(rot)))
}

function paint (state, id, color = 'black') {
  if (!checkBounds(state.animals[id].current.location, state.levelSize)) {
    return state
  }
  return {
    ...state,
    paints: (state.paints || 0) + 1,
    painted: { ...state.painted, [state.animals[id].current.location]: color }
  }
}
...
function paintI (state, id, color = 'black') {
  ...
  return paintMultipleLocations(
    state,
    id,
    color,
    paintLine(location, heading, 4)
  )
}
...
function moveTo (state, id, x, y) {
  if (x === undefined || y === undefined) {
    throw { message: 'Unexpected number of parameters moveTo' }
  }
...
function createRand (state, id, min, max, ...args) {
  if (max === undefined) {
    max = min
    min = 0
  }
  if (!state.rand) {
    return [state, Math.floor(Math.random() * (max - min + 1)) + min]
  }
  return [state, Math.floor(state.rand(max + 1, min))]
}

function randColor (state, colors) {
  const palette =
    colors.length > 0 ? colors : state.palette.map(color => color.name)
  if (!state.rand) {
    return [state, palette[Math.floor(Math.random() * palette.length)]]
  }
  return [state, palette[Math.floor(state.rand(palette.length, 0))]]
}
...
function getCurrentColor (state) {
  ...
  return [state, painted[state.animals[active].current.location] || 'white']
}

function getModdedFrame (state, id, mod) {
  return [state, (state.steps || 0) % mod]
}

function updateRunSpeed (state, id, speed) {
  if (speed < 1 || speed > 100) {
    throw { message: 'Speed should be between 1 and 100.' }
  }
  return { ...state, speed: getUglySpeed(speed) }
}
...
function getUglySpeed (speed) {
  const uglyLower = Math.log(1.2649110640673518)
  const uglyUpper = Math.log(109.95116277760012)
  const uglySpeed = Math.exp(
    (speed - 1) * (uglyUpper - uglyLower) / 99 + uglyLower
  )
  return uglySpeed
}
...
function createPaintFrames (frame, code, end) {
  return filterPaints(createFrames(frame, code, end)).map(f => f.painted)
}

function createFrames (frame, code, end = () => {}) {
  const it = isIterator(code) ? code : createIterator(code)
  const frames = []

  var { value, done } = it.next()
  var isFinished = end(value) || done
...
function updateAnimal (animals = [], path, id, val) {
  return animals.map((animal, i) => {
    return i === id ? setProp(path, animal, val) : animal
  })
}
...
function computeLocation (frame, id, location) {
  return Array.isArray(location)
    ? location
    : location(
      frame.animals[id].current.location,
      frame.animals[id].current.rot
    )
}
  ...
  const frames = createFrames(
    Object.assign({}, state, {
      animals: state.animals.map(a =>
        Object.assign({}, a, { current: a.initial })
      )
    }),
    code
  )
  ...
  return getLastFrame(
    {
      ...
      animals: [
        {
          initial: { rot: 0, location: [state.levelSize[0] - 1, 0] },
          type: 'teacherBot'
        }
      ]
    },
  ...
  return typeof initialPainted === 'object'
    ...
    : getLastFrame(
      ...
      code === undefined
        ? getIterator(
          initialPainted,
          animalApis['teacherBot'].default(active)
        )
        : code
    )
}
...
function getIterator (code, api) {
  ...
  const autoYielded = autoYield(newCode, api, null, 'callFn')
    .split('\n')
    .map((line, i) => {
