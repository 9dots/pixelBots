...
function getFrames (actions, botState) {
	return actions.reduce((arr, next, i) => {
		const prevState = arr[i - 1]
			? arr[i - 1]
			: botState
		return arr.concat(processAction(next, prevState, i + 1))
	}, [])

	function processAction (frame, botState, frameNum) {
		if (!Array.isArray(frame)) frame = [frame]
		return frame.reduce((obj, next) => performAction(obj, getAction(obj, next)), botState, frameNum)
	}

	function performAction (runState, action, frameNum) {
		switch (action.type) {
			case 'animalPaint':
				let {location, color} = action.payload
				return {
					...runState,
					frames: runState.frames.concat({[location]: color, frame: franeNum}),
